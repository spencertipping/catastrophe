...
},ay=typeof caterwaul==="undefined"?y:caterwaul,ax=az();ax.deglobalize=function(){caterwaul=ay;return ax};ax.core_initializer=aw;ax.context=this;ax.merge=(function(aB){for(var aA in aB){if(aB.hasOwnProperty(aA)){return true
...
}}}return aE},ax.modules=[];ax.module=function(aB,aA,aC){if(arguments.length===1){return ax[aB+"_initializer"]}aB+"_initializer" in ax||ax.modules.push(aB);aC||(aC=aA,aA=null);
(ax[aB+"_initializer"]=aA?ax(aA)(aC):aC)(ax);return ax};return caterwaul=ax});var e=function(aw){return aw.split(/\s+/)},ah=function(aw,ax){return ax&&ax.call(aw,aw)||aw
...
},ac=Object.prototype.hasOwnProperty,H=caterwaul.merge(caterwaul,{map:z,rmap:w,gensym:j,is_gensym:f}),D=L(". new ++ -- u++ u-- u+ u- typeof void u~ u! ! * / % + - << >> >>> < > <= >= instanceof in == != === !== & ^ | && || ? = += -= *= /= %= &= |= ^= <<= >>= >>>= : , return throw case var const break continue else u; ;"),ag=function(az){for(var ay=0,ax=[false];
ay<8;++ay){ax.push.apply(ax,ax)}for(var ay=0,aw=az.length;ay<aw;++ay){ax[az.charCodeAt(ay)]=true}return ax},au=ag(".0123456789"),ak=ag("0123456789"),Y=ag("0123456789abcdefABCDEFx"),h=ag("eE"),ai=ag(" \n\r\t"),al=ag("()[]{}?:"),am=ag("([{?:"),g=ag("+-*/%&|^!~=<>?:;.,"),T=ag("\n\r"),K=ag("gims"),o=ag("'\"/"),l="/".charCodeAt(0),i="0".charCodeAt(0),A=L("++ --"),J=ag("@$_abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789"),q="*".charCodeAt(0),d="\\".charCodeAt(0),M="x".charCodeAt(0),R=".".charCodeAt(0),Z="#".charCodeAt(0),ap=z(L,["function","( [ . [] ()","new delete void","u++ u-- ++ -- typeof u~ u! u+ u-","* / %","+ -","<< >> >>>","< > <= >= instanceof in","== != === !==","&","^","|","&&","||","-> =>","case","? = += -= *= /= %= &= |= ^= <<= >>= >>>= &&= ||=",":",",","return throw break continue","var const","if else try catch finally for switch with while do",";"]),u=L("= += -= *= /= %= &= ^= |= <<= >>= >>>= &&= ||= -> => ~ ! new typeof void u+ u- -- ++ u-- u++ ? if else function try catch finally for switch case with while do"),B=(function(ay){for(var aA={},az=0,aw=ay.length;
...
},push:function(aw){return this[this.length++]=H.syntax.promote(aw||aj),this},id:function(){var aw=j("id");return(this.id=function(){return aw})()},is_caterwaul_syntax:true,each:function(ay){for(var ax=0,aw=this.length;
...
az<ay;++az){aA=aA.push(ax[az]).push(az<ay-2?ax.data:ax[az])[1]}}else{for(var az=ax.length-1;az>=1;--az){aA=aA.push(az>1?ax.data:ax[0]).push(ax[az])[0]}}})},as:function(aw){return this.data===aw?this:new H.syntax(aw).push(this)
},bindings:function(ax){var aw=ax||{};this.reach(function(ay){ay.add_bindings_to(aw)});return aw},expressions:function(ax){var aw=ax||{};this.reach(function(ay){ay.add_expressions_to(aw)
});return aw},add_bindings_to:function(aw){},add_expressions_to:function(aw){},resolve:function(){return this},contains:function(az){var aw=az(this);if(aw){return aw
}for(var ay=0,ax=this.length;ay<ax;++ay){if(aw=this[ay].contains(az)){return aw}}},match:function(ay,az){ay=ay.constructor===String?H.parse(ay):ay;az||(az={_:ay});
if(this.is_wildcard()&&(!this.leaf_nodes_only()||!this.length)){return az[this.without_metadata()]=ay,az}else{if(this.length===ay.length&&this.data===ay.data){for(var ax=0,aw=this.length;
ax<aw;++ax){if(!this[ax].match(ay[ax],az)){return null}}return az}}},toString:function(){var aw=[""];this.serialize(aw);return aw.join("")},structure:function(){if(this.length){return"("+['"'+this.data+'"'].concat(z(function(aw){return aw.structure()
},this)).join(" ")+")"}else{return this.data}}};H.syntax_subclasses=[];H.syntax_subclass=function(ay){var ax=Array.prototype.slice.call(arguments,1),aw=function(){return ay.apply(this,arguments)
};H.merge.apply(this,[aw.prototype,n].concat(ax));H.syntax_subclasses.push(aw);aw.prototype.constructor=aw;return aw};H.syntax_extend=function(){for(var ax=0,aw=H.syntax_subclasses.length,ay=Array.prototype.slice.call(arguments);
ax<aw;++ax){H.merge.apply(this,[H.syntax_subclasses[ax].prototype].concat(ay))}H.merge.apply(this,[n].concat(ay));return H};var N=H.parse_hex=function(az){for(var aw=0,ay=0,ax=az.length,aA;
ay<ax;++ay){aw*=16,aw+=(aA=az.charCodeAt(ay))<=58?aA-48:(aA&95)-55}return aw},s=H.parse_octal=function(az){for(var aw=0,ay=0,ax=az.length;ay<ax;++ay){aw*=8,aw+=az.charCodeAt(ay)-48
}return aw},aq=H.unescape_string=function(aA){for(var az=0,aB,ax=aA.length,aw=[],ay=false;az<ax;++az){if(ay){ay=false,aw.push((aB=aA.charAt(az))==="\\"?"\\":aB==="n"?"\n":aB==="r"?"\r":aB==="b"?"\b":aB==="f"?"\f":aB==="0"?"\u0000":aB==="t"?"\t":aB==="v"?"\v":aB==='"'||aB==="'"?aB:aB==="x"?String.fromCharCode(N(aA.substring(az,++az+1))):aB==="u"?String.fromCharCode(N(aA.substring(az,(az+=3)+1))):String.fromCharCode(s(aA.substring(az,(az+=2)+1))))
...
},could_be_identifier:function(){return/^[A-Za-z_$@][A-Za-z0-9$_@]*$/.test(this.data)},is_identifier:function(){return this.length===0&&this.could_be_identifier()&&!this.is_boolean()&&!this.is_null_or_undefined()&&!F(D,this.data)
...
},is_unary_operator:function(){return this.is_prefix_unary_operator()||this.is_postfix_unary_operator()},precedence:function(){return B[this.data]},is_right_associative:function(){return F(u,this.data)
},is_associative:function(){return/^[;,]$/.test(this.data)},is_group:function(){return/^[(\[{][)\]]?$/.test(this.data)},accepts:function(aw){return F(r,this.data)&&r[this.data]===(aw.data||aw)
}};H.javascript_tree_metadata_methods={could_have_metadata:function(){return this.could_be_identifier()},without_metadata:function(){return this.data.replace(/@.*$/g,"")
...
},guarded:function(ay){var az=this.is_group()||this.is_associative()?y:this.precedence(),ax=this.is_right_associative(),aw=this.map(function(aA,aB){return aA.guarded(az-(!ax&&!!aB))
});return az>ay?aw.as("("):aw},serialize:function(az){var ax=this.length,aA=this.data,aw=";\n",ay=function(aB){if(J[az[az.length-1].charCodeAt(0)]===J[aB.charCodeAt(0)]){az.push(" ",aB)
}else{az.push(aB)}};switch(ax){case 0:if(F(av,aA)){return ay(aA.replace(/^u/,""))}else{if(F(at,aA)){return ay(aA),ay(at[aA])}else{return ay(aA)}}case 1:if(F(O,aA)||F(av,aA)){return ay(aA.replace(/^u/,"")),this[0].serialize(az)
...
}else{return this.unflatten().serialize(az)}}}}};H.ref_common=H.merge({},H.javascript_tree_type_methods,H.javascript_tree_metadata_methods,H.javascript_tree_serialization_methods,{replace:function(aw){var ax;
...
}},{resolve:function(){return this.expression_refs?H.late_bound_tree(new this.constructor(this.data),this.expression_refs):this},serialize:function(aw){return aw.push(this.data),aw
},parse:function(){return H.parse(this.data)}});H.syntax=ah(H.syntax_subclass(function(aB){if(aB instanceof this.constructor){this.data=aB.data,this.length=0}else{this.data=aB&&aB.toString();
this.length=0;for(var az=1,aw=arguments.length,ay;ay=arguments[az],az<aw;++az){for(var ax=0,aD=ay.length,aA,aC;ay instanceof Array?(aA=ay[ax],ax<aD):(aA=ay,!ax);
++ax){this._append(H.syntax.promote(aA))}}}},H.javascript_tree_type_methods,H.javascript_tree_metadata_methods,H.javascript_tree_serialization_methods),function(){this.from_string=function(aw){return new H.syntax('"'+aw.replace(/\\/g,"\\\\").replace(/"/g,'\\"').replace(/\n/g,"\\n")+'"')
};this.from_array=function(ax){for(var ay=0,az=new H.syntax(","),aw=ax.length;ay<aw;++ay){az.push(ax[ay])}return new H.syntax("[",az.length?az.unflatten():[])};this.from_object=function(ay){var aw=new H.syntax(",");
for(var ax in ay){if(ac.call(ay,ax)){aw.push(new H.syntax(":",/^[$_A-Za-z][A-Za-z0-9$_]*$/.test(ax)?ax:H.syntax.from_string(ax),ay[ax].as("(")))}}return new H.syntax("{",aw.length?aw.unflatten():[])
}});H.syntax.promote=function(aw){var ax=aw.constructor;return ax===String||ax===Number||ax===Boolean?new H.syntax(aw):aw};var aj=H.empty=new H.syntax("");H.parse=function(aw){if(aw.constructor===H.syntax){return aw
}var aR=aw.toString(),aB=0,a3=0,aO=true,aN=false,a5=false,a4=false,aS=0,aP="",a0=0,aX=aR.length,aI=function(a7){return aR.charCodeAt(a7)},aG=[],aW=null,aE=null,aC=null,ax=z(function(){return[]
},ap),aD=[],aQ=[aj],aY=function(a7){return aQ.push(a7),a7},aT=function(a7){return aE?aE._sibling(aE=a7):(aE=a7._append_to(aC)),aY(a7)},ay=this.syntax,aF=[];if(aX===0){return aj
}while((aB=a0)<aX){while(ai[a3=aI(a0)]&&a0<aX){aB=++a0}aN=a4=a5=aP=false;if(al[a3]){aP=!!++a0;aO=am[a3]}else{if(a3===l&&aI(a0+1)===q&&(a0+=2)){while(++a0<aX&&aI(a0)!==l||aI(a0-1)!==q){}aP=!++a0
...
aZ+=a1){if(F(aa,a2=(aJ=a6[aZ]).data)){if(a2===":"&&B[aJ.r.data]>a0){aJ._fold_l()}else{aJ._fold_lr()}}else{if(F(Q,a2)&&aJ.l&&!((aK=aJ.l.l)&&F(m,aK.data))&&(aJ.l.data==="."||(aJ.l.data==="function"&&aJ.l.length===2)||!(F(D,aJ.l.data)||F(I,aJ.l.data)))){aD.push(aJ.l._wrap(aY(new ay(a2+at[a2]))).p._fold_r())
...
H.compile=function(aH,aB,aG){aG=H.merge({gensym_renaming:true,transparent_errors:false},aG);aH=H.late_bound_tree(aH,null,aG);var ay=H.merge({},this._environment,aB,aH.bindings()),aD=[k],aF=j("base");
...
if(aG.transparent_errors){return(new Function(aF,ax)).call(ay["this"],ay)}else{try{return(new Function(aF,ax)).call(ay["this"],ay)}catch(aC){throw new Error((aC.message||aC)+" while compiling "+ax)
}}};var G=H.parse("new caterwaul.syntax(_data)"),t=H.parse("new caterwaul.syntax(_data, _xs)");H.syntax_to_expression=function(ax){if(ax.length){for(var aw=new H.syntax(","),az=0,ay=ax.length;
az<ay;++az){aw.push(H.syntax_to_expression(ax[az]))}return t.replace({_data:H.syntax.from_string(ax.data),_xs:aw.unflatten()})}else{return G.replace({_data:H.syntax.from_string(ax.data)})
}};H.late_bound_tree=function(aF,az,aE){aE=H.merge({expression_ref_table:true},aE);aF=aF.rmap(function(aG){return aG.resolve()});var aw=H.merge({},az,aF.expressions()),aC=new H.syntax(","),aA=new H.syntax(","),aD={};
for(var ay in aw){if(ac.call(aw,ay)){aC.push(new H.syntax(ay)),aA.push(aw[ay]),aD[ay]=H.syntax.from_string(aw[ay].toString())}}var aB=H.gensym("result"),ax=aE.expression_ref_table?V.replace({_result:aB,_expression_ref_table:H.syntax.from_object(aD)}):H.empty;
...
}return ax};var b=function(ax){/^:/.test(ax)&&(ax=H[ax.substr(1)]);ax.constructor===String&&(ax=ax.split(/\s+/));for(var ay=1,aw=ax.length,az=H[ax[0]]();ay<aw;++ay){az=H[ax[ay]](az)
}return az};H.init=function(aw){aw||(aw=function(ax){return true});return aw.constructor===Function?ah((function(){var ax=function(aA,ay,az){return typeof aA==="function"||aA.constructor===String?H.compile(ax.call(ax,H.parse(aA)),ay,az):aA.rmap(function(aB){return aw.call(ax,aB,ay,az)
...
}if(az&&az.core_only){return ao.replace({_x:new this.opaque_tree(this.initializer)})}for(var aA=0,ay=az&&az.modules||this.modules,aB=[],ax=ay.length;aA<ax;++aA){aB.push(E.replace({_name:this.syntax.from_string(ay[aA]),_f:new this.opaque_tree(this.module(ay[aA]))}))
}for(var aA=0,ax=aB.length,aw=new this.syntax(".",ao.replace({_x:new this.opaque_tree(this.initializer)}));aA<ax;++aA){aw.push(aB[aA])}return this.late_bound_tree(aw.unflatten())
...
}return f}).call(this,d))}}}))}).call(this)});caterwaul.module("catastrophe",(function(a,ae,ac,aa,Y,W,U,S,Q,O,K,J,I,H,G,F,E,D,C,B,A,z,y,x,w,v,u,t,s,r,q,p,o,n,m,l,k,j,i,h,g,f,e,d,c,b,bm,bl,bk,bj,bi,bh,bg,bf,be,bd,bc,bb,by,au,ba,a9,a7,a5,a3,a1,aZ,aX,aV,aT,aR,aP,aO,aN,bx,bw,bv,bu,bt,bs,bq,bp,bo,bn,aM,aL,aK,aJ,aI,aH,aG,aF,aE,aD,aC,aB,aA,az,ay,ax,aw,av,at,ar,aq,ap,ao,an,am,al,a8,a6,a4,a2,a0,aY,aW,aU,aS,aQ,ak,aj,ai,ah,ag,af,ad,ab,Z,X,V,T,R,P,N,M,L){var br=(function(bz){(function(){var bE=bz.context,bJ={caterwaul:bz.deglobalize()},bF=function(bK){return(function(){var bQ={mocks:{},allow_mock_annotations:true,trace_native_eval_forms:true,environment:{},pre_trace:true,trace:true,timestamps:true,hook_name:bz.gensym("catastrophe_hook"),hook:null,global:bE,trace_log_size:1<<20},bN=bz.merge({},bQ,bK),bL=function(){return bP.apply(this,arguments)
},bM=bN.trace_native_eval_forms?bG(bL):{},bO=bz.merge(bM,bN.mocks),bP=(function(bR){return(bR.install_hook()),bR}).call(this,(bz.merge(bC(bz.merge(bN,{mocks:bO,trace_log:[],error_log:[]})),(function(){var bU=bN,bX=function(bZ){return(function(){var b0=bz.parse(bZ);
return(bH((function(b3){var b1,b5,b2,b6,b4;for(var b4=new b3.constructor(),b2=0,b6=b3.length;b2<b6;++b2){b1=b3[b2],((b0).match(b1.tree))&&b4.push(b1)}return b4}).call(this,bW),b0))
}).call(this)},bY=function(){return(function(b1){var bZ,b3,b0,b4,b2;for(var b2=new b1.constructor(),b0=0,b4=b1.length;b0<b4;++b0){bZ=b1[b0],b2.push((bH(bZ,"_x")))
}return b2}).call(this,bR)},bW=bU.trace_log,bR=bU.error_log,bV=!!bU.hook||(bU.hook=bA(bU)),bT=function(){return bU.global&&(bU.global[bU.hook_name]=bV)},bS=function(){return bU.global&&delete bU.global[bU.hook_name]
};return({options:bU,find:bX,errors:bY,trace_log:bW,error_log:bR,hook:bV,install_hook:bT,remove_hook:bS})}).call(this))));return(bP)}).call(this)},bH=function(bK,bL){return(function(bM){return(bM.match_context=bL),bM
}).call(this,(bz.merge(bK,bI)))},bI=(function(){var bL=bz(":all"),bO=function(bP){return bP.constructor===Function?bP:this.compile_string(bP)},bM=function(bP){return bL(("given.x ["+(bP)+", where [match = match_context /~match/ x.tree]]"),{match_context:this.match_context})
},bN=function(bP){return(function(){var bQ=this.promote(bP);return(bH((function(bV){var bR,bU,bW,bS,bT;for(var bT=new bV.constructor(),bW=0,bS=bV.length;bW<bS;++bW){bR=bV[bW],bT.push((bQ(bR)))
}return bT}).call(this,this),this.match_context))}).call(this)},bK=function(bP){return(function(){var bQ=this.promote(bP);return(bH((function(bV){var bR,bU,bW,bS,bT;
for(var bT=new bV.constructor(),bW=0,bS=bV.length;bW<bS;++bW){bR=bV[bW],(bQ(bR))&&bT.push(bR)}return bT}).call(this,this),this.match_context))}).call(this)};return({function_compiler:bL,promote:bO,compile_string:bM,map:bN,filter:bK})
}).call(this),bD=function(bK){return(function(){var bL=function(bN,bM){return(function(){var bP=function(){return bK.trace?(((((bY).concat((bS))).concat((bT))).concat((bV))).concat((b0))).concat((b1)):[]
},bX=function(){return(function(){var b3=bK.allow_mock_annotations?bM:function(b4){return b4},b2=function(b4){return b4 instanceof Function?b4:b3(bz.parse(b4))};
return((function(b6){var b4,b8,b5,b9,b7;for(var b7=new b6.constructor(),b5=0,b9=b6.length;b5<b9;++b5){b4=b6[b5],b7.push((bN(bM(bz.parse(b4[0])),b2(b2(b4[1])))))}return b7
}).call(this,(function(b6){var b5=[];for(var b4 in b6){Object.prototype.hasOwnProperty.call(b6,b4)&&b5.push([b4,b6[b4]])}return b5}).call(this,(bK.mocks))))}).call(this)
},bY=[bN(a,ae)],bS=[bN(ac,aa),bN(Y,W),bN(U,S),bN(Q,O),bN(K,J),bN(I,H),bN(G,F),bN(E,D),bN(C,B),bN(A,z),bN(y,x),bN(w,v),bN(u,t),bN(s,r),bN(q,p),bN(o,n),bN(m,l),bN(k,j),bN(i,h),bN(g,f),bN(e,d),bN(c,b),bN(bm,bl),bN(bk,bj)],bT=[bN(bi,bh),bN(bg,bf),bN(be,bd),bN(bc,bb)],bV=(function(){var b3=by,b2=au;
return((([bN(ba,a9),bN(a7,a5),bN(a3,a1),bN(aZ,aX),bN(aV,aT),bN(aR,aP),bN(aO,aN),bN(bx,bw),bN(bv,bu),bN(bt,bs),bN(bq,bp),bN(bo,bn),bN(aM,aL),bN(aK,aJ),bN(aI,aH),bN(aG,aF),bN(aE,aD),bN(aC,aB),bN(aA,az),bN(ay,ax),bN(aw,av),bN(at,ar),bN(aq,ap),bN(ao,an),bN(am,al)]).concat(((function(b6){var b4,b8,b5,b9,b7;
for(var b7=new b6.constructor(),b5=0,b9=b6.length;b5<b9;++b5){b4=b6[b5],b7.push((bN(b4[0],b4[1])))}return b7}).call(this,((b3).concat((b2))))))).concat(([bN(a8,a6),bN(a4,a2),bN(a0,aY),bN(aW,aU),bN(aS,aQ),bN(ak,aj),bN(ai,ah),bN(ag,af),bN(ad,ab)])))
}).call(this),bU=new bz.syntax(bK.hook_name),bO=(Z).replace({h:bU}),bZ=(X).replace({h:bU}),bQ={},bR=function(b2){return !!bQ[b2.id()]||(bQ[b2.id()]=b2.rmap((function(b3){return b3.data==="[]"&&bz.is_gensym(b3[0].data)?bR(b3[1]):false
})))},bW=function(b2){return((bK.pre_trace?bO:bZ)).replace({_x:new bz.ref(bR(b2)),_value:b2})},b0=[bN(V,function(b2){return bW(b2._x)})],b1=[bN(T,R)];return((bP()).concat((bX())))
}).call(this)};return(bz.grammar(["L","U","R","C","H","S"],{initial:P},bL))}).call(this)},bA=function(bK){return(function(){var bM=function(bV,bU){return arguments.length===2?bS(bV,bU):bL(bV)
},bL=function(bU){return(bP).push({tree:bU,time:bK.timestamp&&+new Date()})},bS=function(bV,bU){return(((((bN).push({tree:bV,value:bU,time:bK.timestamp&&+new Date(),pre_trace:bK.pre_trace&&bO(bV)})),(((bN.length>bK.trace_log_size<<1)&&(bT()))))),(bU))
},bO=function(bU){return bP.length&&bP[bP.length-1].tree===bU?bP.pop():bQ(bU)},bQ=function(bU){return(function(){var bV=(function(b1){var bY,b3,b0,b4,b2;for(var b4=b1.length,b0=b4-1,bY=b1[b0],bZ;
b0>=0;--b0){bY=b1[b0];if(b3=(bY.tree===bU)){return b0}}return false}).call(this,bP),bX=bP[bV],bW=bP.splice(bV,bP.length-bV);return((((bR).push(bW)),(bX)))}).call(this)
},bT=function(){return bN.splice(bN.length-bK.trace_log_size,bK.trace_log_size)},bP=bK.pre_trace&&[],bN=bK.trace_log,bR=bK.pre_trace&&!!bK.error_log||(bK.error_log=[]);
return(bM)}).call(this)},bB=function(bK){return function(bL){return bK(bz.parse(bL)).toString()}},bG=function(bK){return{"R[eval(_x)]":(N).replace({trace:new bz.ref(bB(bK))}),"R[new Function(_xs)]":(function(){var bO=M,bM=(L).replace({trace:new bz.ref(bB(bK))}),bN=function(bP){return bP.flatten(",").map((function(bQ,bR){return bR?bQ:bM.replace({_x:bQ})
})).unflatten()},bL=function(bP){return(bO).replace({_xs:bN(bP._xs)})};return(bL)}).call(this)}},bC=function(bK){return(function(){var bN=bD(bK),bM=bz(function(bO){return bN({_x:bO})
}),bL=function(bP,bO){return bM(bP,bz.merge({},bK.environment,bO),{transparent_errors:false,gensym_renaming:false})};return(bL)}).call(this)};return(bz.merge((catastrophe=bF),bJ))
}).call(this)});br.caterwaul_expression_ref_table={qs:('new caterwaul.syntax( "[]" ,new caterwaul.syntax( "U" ) ,new caterwaul.syntax( "_x" ) )'),qs1:('new caterwaul.syntax( "" )'),qs2:('new caterwaul.syntax( "[]" ,new caterwaul.syntax( "S" ) ,new caterwaul.syntax( "_x" ) )'),qs3:('new caterwaul.syntax( "[]" ,new caterwaul.syntax( "R" ) ,new caterwaul.syntax( "_x" ) )'),qs4:('new caterwaul.syntax( "[]" ,new caterwaul.syntax( "S" ) ,new caterwaul.syntax( "{" ,new caterwaul.syntax( "_x" ) ) )'),qs5:('new caterwaul.syntax( "{" ,new caterwaul.syntax( "[]" ,new caterwaul.syntax( "S" ) ,new caterwaul.syntax( "_x" ) ) )'),qs6:('new caterwaul.syntax( "[]" ,new caterwaul.syntax( "S" ) ,new caterwaul.syntax( "i;" ,new caterwaul.syntax( "_x" ) ,new caterwaul.syntax( "_y" ) ) )'),qs7:('new caterwaul.syntax( "i;" ,new caterwaul.syntax( "[]" ,new caterwaul.syntax( "S" ) ,new caterwaul.syntax( "_x" ) ) ,new caterwaul.syntax( "[]" ,new caterwaul.syntax( "S" ) ,new caterwaul.syntax( "_y" ) ) )'),qs8:('new caterwaul.syntax( "[]" ,new caterwaul.syntax( "S" ) ,new caterwaul.syntax( ";" ,new caterwaul.syntax( "_x" ) ,new caterwaul.syntax( "_y" ) ) )'),qs9:('new caterwaul.syntax( ";" ,new caterwaul.syntax( "[]" ,new caterwaul.syntax( "S" ) ,new caterwaul.syntax( "_x" ) ) ,new caterwaul.syntax( "[]" ,new caterwaul.syntax( "S" ) ,new caterwaul.syntax( "_y" ) ) )'),qsa:('new caterwaul.syntax( "[]" ,new caterwaul.syntax( "S" ) ,new caterwaul.syntax( "var" ,new caterwaul.syntax( "," ,new caterwaul.syntax( "_vs" ) ,new caterwaul.syntax( "=" ,new caterwaul.syntax( "_name" ) ,new caterwaul.syntax( "_value" ) ) ) ) )'),qsb:('new caterwaul.syntax( ";" ,new caterwaul.syntax( "[]" ,new caterwaul.syntax( "S" ) ,new caterwaul.syntax( "var" ,new caterwaul.syntax( "_vs" ) ) ) ,new caterwaul.syntax( "[]" ,new caterwaul.syntax( "S" ) ,new caterwaul.syntax( "var" ,new caterwaul.syntax( "=" ,new caterwaul.syntax( "_name" ) ,new caterwaul.syntax( "_value" ) ) ) ) )'),qsc:('new caterwaul.syntax( "[]" ,new caterwaul.syntax( "S" ) ,new caterwaul.syntax( "var" ,new caterwaul.syntax( "=" ,new caterwaul.syntax( "_name" ) ,new caterwaul.syntax( "_value" ) ) ) )'),qsd:('new caterwaul.syntax( "var" ,new caterwaul.syntax( "=" ,new caterwaul.syntax( "[]" ,new caterwaul.syntax( "L" ) ,new caterwaul.syntax( "_name" ) ) ,new caterwaul.syntax( "[]" ,new caterwaul.syntax( "R" ) ,new caterwaul.syntax( "_value" ) ) ) )'),qse:('new caterwaul.syntax( "[]" ,new caterwaul.syntax( "S" ) ,new caterwaul.syntax( "for" ,new caterwaul.syntax( "(" ,new caterwaul.syntax( ";" ,new caterwaul.syntax( ";" ,new caterwaul.syntax( "_x" ) ,new caterwaul.syntax( "_y" ) ) ,new caterwaul.syntax( "_z" ) ) ) ,new caterwaul.syntax( "_body" ) ) )'),qsf:('new caterwaul.syntax( "for" ,new caterwaul.syntax( "(" ,new caterwaul.syntax( ";" ,new caterwaul.syntax( ";" ,new caterwaul.syntax( "[]" ,new caterwaul.syntax( "S" ) ,new caterwaul.syntax( "_x" ) ) ,new caterwaul.syntax( "[]" ,new caterwaul.syntax( "R" ) ,new caterwaul.syntax( "_y" ) ) ) ,new caterwaul.syntax( "[]" ,new caterwaul.syntax( "R" ) ,new caterwaul.syntax( "_z" ) ) ) ) ,new caterwaul.syntax( "[]" ,new caterwaul.syntax( "S" ) ,new caterwaul.syntax( "_body" ) ) )'),qsg:('new caterwaul.syntax( "[]" ,new caterwaul.syntax( "S" ) ,new caterwaul.syntax( "for" ,new caterwaul.syntax( "(" ,new caterwaul.syntax( "var" ,new caterwaul.syntax( "in" ,new caterwaul.syntax( "_name" ) ,new caterwaul.syntax( "_o" ) ) ) ) ,new caterwaul.syntax( "_body" ) ) )'),qsh:('new caterwaul.syntax( "for" ,new caterwaul.syntax( "(" ,new caterwaul.syntax( "var" ,new caterwaul.syntax( "in" ,new caterwaul.syntax( "[]" ,new caterwaul.syntax( "L" ) ,new caterwaul.syntax( "_name" ) ) ,new caterwaul.syntax( "[]" ,new caterwaul.syntax( "R" ) ,new caterwaul.syntax( "_o" ) ) ) ) ) ,new caterwaul.syntax( "[]" ,new caterwaul.syntax( "S" ) ,new caterwaul.syntax( "_body" ) ) )'),qsi:('new caterwaul.syntax( "[]" ,new caterwaul.syntax( "S" ) ,new caterwaul.syntax( "for" ,new caterwaul.syntax( "(" ,new caterwaul.syntax( "var" ,new caterwaul.syntax( "," ,new caterwaul.syntax( "in" ,new caterwaul.syntax( "_name" ) ,new caterwaul.syntax( "_1" ) ) ,new caterwaul.syntax( "_2" ) ) ) ) ,new caterwaul.syntax( "_body" ) ) )'),qsj:('new caterwaul.syntax( "for" ,new caterwaul.syntax( "(" ,new caterwaul.syntax( "var" ,new caterwaul.syntax( "," ,new caterwaul.syntax( "in" ,new caterwaul.syntax( "[]" ,new caterwaul.syntax( "L" ) ,new caterwaul.syntax( "_name" ) ) ,new caterwaul.syntax( "[]" ,new caterwaul.syntax( "R" ) ,new caterwaul.syntax( "_1" ) ) ) ,new caterwaul.syntax( "[]" ,new caterwaul.syntax( "R" ) ,new caterwaul.syntax( "_2" ) ) ) ) ) ,new caterwaul.syntax( "_body" ) )'),qsk:('new caterwaul.syntax( "[]" ,new caterwaul.syntax( "S" ) ,new caterwaul.syntax( "for" ,new caterwaul.syntax( "(" ,new caterwaul.syntax( "var" ,new caterwaul.syntax( "=" ,new caterwaul.syntax( "in" ,new caterwaul.syntax( "_name" ) ,new caterwaul.syntax( "_1" ) ) ,new caterwaul.syntax( "_2" ) ) ) ) ,new caterwaul.syntax( "_body" ) ) )'),qsl:('new caterwaul.syntax( "for" ,new caterwaul.syntax( "(" ,new caterwaul.syntax( "var" ,new caterwaul.syntax( "=" ,new caterwaul.syntax( "in" ,new caterwaul.syntax( "[]" ,new caterwaul.syntax( "L" ) ,new caterwaul.syntax( "_name" ) ) ,new caterwaul.syntax( "[]" ,new caterwaul.syntax( "L" ) ,new caterwaul.syntax( "_1" ) ) ) ,new caterwaul.syntax( "[]" ,new caterwaul.syntax( "R" ) ,new caterwaul.syntax( "_2" ) ) ) ) ) ,new caterwaul.syntax( "_body" ) )'),qsm:('new caterwaul.syntax( "[]" ,new caterwaul.syntax( "S" ) ,new caterwaul.syntax( "do" ,new caterwaul.syntax( "_body" ) ,new caterwaul.syntax( "while" ,new caterwaul.syntax( "(" ,new caterwaul.syntax( "_cond" ) ) ) ) )'),qsn:('new caterwaul.syntax( "do" ,new caterwaul.syntax( "S" ) ,new caterwaul.syntax( "[" ,new caterwaul.syntax( "_body" ) ) ,new caterwaul.syntax( "while" ,new caterwaul.syntax( "(" ,new caterwaul.syntax( "[]" ,new caterwaul.syntax( "R" ) ,new caterwaul.syntax( "_cond" ) ) ) ) )'),qso:('new caterwaul.syntax( "[]" ,new caterwaul.syntax( "S" ) ,new caterwaul.syntax( "while" ,new caterwaul.syntax( "(" ,new caterwaul.syntax( "_cond" ) ) ,new caterwaul.syntax( "_body" ) ) )'),qsp:('new caterwaul.syntax( "while" ,new caterwaul.syntax( "(" ,new caterwaul.syntax( "[]" ,new caterwaul.syntax( "R" ) ,new caterwaul.syntax( "_cond" ) ) ) ,new caterwaul.syntax( "[]" ,new caterwaul.syntax( "S" ) ,new caterwaul.syntax( "_body" ) ) )'),qsq:('new caterwaul.syntax( "[]" ,new caterwaul.syntax( "S" ) ,new caterwaul.syntax( "i;" ,new caterwaul.syntax( "()" ,new caterwaul.syntax( "with" ) ,new caterwaul.syntax( "_x" ) ) ,new caterwaul.syntax( "_body" ) ) )'),qsr:('new caterwaul.syntax( "i;" ,new caterwaul.syntax( "()" ,new caterwaul.syntax( "with" ) ,new caterwaul.syntax( "[]" ,new caterwaul.syntax( "R" ) ,new caterwaul.syntax( "_x" ) ) ) ,new caterwaul.syntax( "[]" ,new caterwaul.syntax( "S" ) ,new caterwaul.syntax( "_body" ) ) )'),qss:('new caterwaul.syntax( "[]" ,new caterwaul.syntax( "S" ) ,new caterwaul.syntax( "if" ,new caterwaul.syntax( "(" ,new caterwaul.syntax( "_cond" ) ) ,new caterwaul.syntax( "_body" ) ) )'),qst:('new caterwaul.syntax( "if" ,new caterwaul.syntax( "(" ,new caterwaul.syntax( "[]" ,new caterwaul.syntax( "R" ) ,new caterwaul.syntax( "_cond" ) ) ) ,new caterwaul.syntax( "[]" ,new caterwaul.syntax( "S" ) ,new caterwaul.syntax( "_body" ) ) )'),qsu:('new caterwaul.syntax( "[]" ,new caterwaul.syntax( "S" ) ,new caterwaul.syntax( "if" ,new caterwaul.syntax( "(" ,new caterwaul.syntax( "_cond" ) ) ,new caterwaul.syntax( "_body" ) ,new caterwaul.syntax( "else" ,new caterwaul.syntax( "_else" ) ) ) )'),qsv:('new caterwaul.syntax( "if" ,new caterwaul.syntax( "(" ,new caterwaul.syntax( "[]" ,new caterwaul.syntax( "R" ) ,new caterwaul.syntax( "_cond" ) ) ) ,new caterwaul.syntax( "[]" ,new caterwaul.syntax( "S" ) ,new caterwaul.syntax( "_body" ) ) ,new caterwaul.syntax( "else" ,new caterwaul.syntax( "[]" ,new caterwaul.syntax( "S" ) ,new caterwaul.syntax( "_else" ) ) ) )'),qsw:('new caterwaul.syntax( "[]" ,new caterwaul.syntax( "S" ) ,new caterwaul.syntax( "try" ,new caterwaul.syntax( "_x" ) ,new caterwaul.syntax( "catch" ,new caterwaul.syntax( "_y" ) ,new caterwaul.syntax( "finally" ,new caterwaul.syntax( "_z" ) ) ) ) )'),qsx:('new caterwaul.syntax( "try" ,new caterwaul.syntax( "S" ) ,new caterwaul.syntax( "[" ,new caterwaul.syntax( "_x" ) ) ,new caterwaul.syntax( "catch" ,new caterwaul.syntax( "S" ) ,new caterwaul.syntax( "[" ,new caterwaul.syntax( "_y" ) ) ,new caterwaul.syntax( "finally" ,new caterwaul.syntax( "[]" ,new caterwaul.syntax( "S" ) ,new caterwaul.syntax( "_z" ) ) ) ) )'),qsy:('new caterwaul.syntax( "[]" ,new caterwaul.syntax( "S" ) ,new caterwaul.syntax( "try" ,new caterwaul.syntax( "_x" ) ,new caterwaul.syntax( "catch" ,new caterwaul.syntax( "_y" ) ) ) )'),qsz:('new caterwaul.syntax( "try" ,new caterwaul.syntax( "S" ) ,new caterwaul.syntax( "[" ,new caterwaul.syntax( "_x" ) ) ,new caterwaul.syntax( "catch" ,new caterwaul.syntax( "S" ) ,new caterwaul.syntax( "[" ,new caterwaul.syntax( "_y" ) ) ) )'),qs10:('new caterwaul.syntax( "[]" ,new caterwaul.syntax( "S" ) ,new caterwaul.syntax( "try" ,new caterwaul.syntax( "_x" ) ,new caterwaul.syntax( "finally" ,new caterwaul.syntax( "_y" ) ) ) )'),qs11:('new caterwaul.syntax( "i;" ,new caterwaul.syntax( "try" ,new caterwaul.syntax( "S" ) ,new caterwaul.syntax( "[" ,new caterwaul.syntax( "_x" ) ) ) ,new caterwaul.syntax( "finally" ,new caterwaul.syntax( "[]" ,new caterwaul.syntax( "S" ) ,new caterwaul.syntax( "_y" ) ) ) )'),qs12:('new caterwaul.syntax( "[]" ,new caterwaul.syntax( "S" ) ,new caterwaul.syntax( "throw" ,new caterwaul.syntax( "_x" ) ) )'),qs13:('new caterwaul.syntax( "throw" ,new caterwaul.syntax( "[]" ,new caterwaul.syntax( "R" ) ,new caterwaul.syntax( "_x" ) ) )'),qs14:('new caterwaul.syntax( "[]" ,new caterwaul.syntax( "S" ) ,new caterwaul.syntax( "return" ,new caterwaul.syntax( "_x" ) ) )'),qs15:('new caterwaul.syntax( "return" ,new caterwaul.syntax( "[]" ,new caterwaul.syntax( "R" ) ,new caterwaul.syntax( "_x" ) ) )'),qs16:('new caterwaul.syntax( "[]" ,new caterwaul.syntax( "S" ) ,new caterwaul.syntax( ";" ,new caterwaul.syntax( "throw" ,new caterwaul.syntax( "_x" ) ) ,new caterwaul.syntax( "_y" ) ) )'),qs17:('new caterwaul.syntax( ";" ,new caterwaul.syntax( "[]" ,new caterwaul.syntax( "S" ) ,new caterwaul.syntax( "throw" ,new caterwaul.syntax( "_x" ) ) ) ,new caterwaul.syntax( "[]" ,new caterwaul.syntax( "U" ) ,new caterwaul.syntax( "_y" ) ) )'),qs18:('new caterwaul.syntax( "[]" ,new caterwaul.syntax( "S" ) ,new caterwaul.syntax( ";" ,new caterwaul.syntax( "return" ,new caterwaul.syntax( "_x" ) ) ,new caterwaul.syntax( "_y" ) ) )'),qs19:('new caterwaul.syntax( ";" ,new caterwaul.syntax( "[]" ,new caterwaul.syntax( "S" ) ,new caterwaul.syntax( "return" ,new caterwaul.syntax( "_x" ) ) ) ,new caterwaul.syntax( "[]" ,new caterwaul.syntax( "U" ) ,new caterwaul.syntax( "_y" ) ) )'),qs1a:('new caterwaul.syntax( "[]" ,new caterwaul.syntax( "S" ) ,new caterwaul.syntax( "i;" ,new caterwaul.syntax( "()" ,new caterwaul.syntax( "switch" ) ,new caterwaul.syntax( "_value" ) ) ,new caterwaul.syntax( "{" ,new caterwaul.syntax( "_cases" ) ) ) )'),qs1b:('new caterwaul.syntax( "i;" ,new caterwaul.syntax( "()" ,new caterwaul.syntax( "switch" ) ,new caterwaul.syntax( "[]" ,new caterwaul.syntax( "R" ) ,new caterwaul.syntax( "_value" ) ) ) ,new caterwaul.syntax( "{" ,new caterwaul.syntax( "[]" ,new caterwaul.syntax( "S" ) ,new caterwaul.syntax( "_cases" ) ) ) )'),qs1c:('new caterwaul.syntax( "[]" ,new caterwaul.syntax( "S" ) ,new caterwaul.syntax( ":" ,new caterwaul.syntax( "_x" ) ,new caterwaul.syntax( "_y" ) ) )'),qs1d:('new caterwaul.syntax( ":" ,new caterwaul.syntax( "_x" ) ,new caterwaul.syntax( "[]" ,new caterwaul.syntax( "S" ) ,new caterwaul.syntax( "_y" ) ) )'),qs1e:('new caterwaul.syntax( "[]" ,new caterwaul.syntax( "L" ) ,new caterwaul.syntax( "_x@0" ) )'),qs1f:('new caterwaul.syntax( "_x" )'),qs1g:('new caterwaul.syntax( "[]" ,new caterwaul.syntax( "L" ) ,new caterwaul.syntax( "[]" ,new caterwaul.syntax( "_x" ) ,new caterwaul.syntax( "_y" ) ) )'),qs1h:('new caterwaul.syntax( "[]" ,new caterwaul.syntax( "[]" ,new caterwaul.syntax( "R" ) ,new caterwaul.syntax( "_x" ) ) ,new caterwaul.syntax( "[]" ,new caterwaul.syntax( "L" ) ,new caterwaul.syntax( "_y" ) ) )'),qs1i:('new caterwaul.syntax( "[]" ,new caterwaul.syntax( "L" ) ,new caterwaul.syntax( "." ,new caterwaul.syntax( "_x" ) ,new caterwaul.syntax( "_y" ) ) )'),qs1j:('new caterwaul.syntax( "." ,new caterwaul.syntax( "[]" ,new caterwaul.syntax( "R" ) ,new caterwaul.syntax( "_x" ) ) ,new caterwaul.syntax( "_y" ) )'),qs1k:('new caterwaul.syntax( "[]" ,new caterwaul.syntax( "L" ) ,new caterwaul.syntax( "()" ,new caterwaul.syntax( "_x" ) ,new caterwaul.syntax( "_y" ) ) )'),qs1l:('new caterwaul.syntax( "[]" ,new caterwaul.syntax( "U" ) ,new caterwaul.syntax( "()" ,new caterwaul.syntax( "_x" ) ,new caterwaul.syntax( "_y" ) ) )'),ahead:("(function(xs) {var x,x0,xi,xl,xr;\nfor(var xr=new xs.constructor() ,xi=0,xl=xs.length;\nxi<xl;\n ++xi)x=xs[xi] ,xr.push( ( [caterwaul.parse( ( 'R[_x ' + (x) + ' _y]' ) ) ,caterwaul.parse( ( 'H[_x ' + (x) + ' _y, R[_x] ' + (x) + ' R[_y]]' ) ) ] ) ) ;\nreturn xr} ) .call(this, [ '+' , '-' , '*' , '/' , '%' , '<<' , '>>' , '>>>' , '<' , '>' , '<=' , '>=' , 'instanceof' , 'in' , '==' , '!=' , '===' , '!==' , '&' , '^' , '|' , '&&' , '||' ] )"),ahead1:("(function(xs) {var x,x0,xi,xl,xr;\nfor(var xr=new xs.constructor() ,xi=0,xl=xs.length;\nxi<xl;\n ++xi)x=xs[xi] ,xr.push( ( [caterwaul.parse( ( 'R[_x ' + (x) + ' _y]' ) ) ,caterwaul.parse( ( 'H[_x ' + (x) + ' _y, L[_x] ' + (x) + ' R[_y]]' ) ) ] ) ) ;\nreturn xr} ) .call(this, [ '=' , '+=' , '-=' , '*=' , '/=' , '%=' , '<<=' , '>>=' , '>>>=' , '&=' , '|=' , '^=' ] )"),qs1m:('new caterwaul.syntax( "[]" ,new caterwaul.syntax( "R" ) ,new caterwaul.syntax( "_x@0" ) )'),qs1n:('new caterwaul.syntax( "[]" ,new caterwaul.syntax( "H" ) ,new caterwaul.syntax( "_x" ) )'),qs1o:('new caterwaul.syntax( "[]" ,new caterwaul.syntax( "R" ) ,new caterwaul.syntax( "" ) )'),qs1p:('new caterwaul.syntax( "" )'),qs1q:('new caterwaul.syntax( "[]" ,new caterwaul.syntax( "R" ) ,new caterwaul.syntax( "delete" ,new caterwaul.syntax( "_x" ) ) )'),qs1r:('new caterwaul.syntax( "[]" ,new caterwaul.syntax( "H" ) ,new caterwaul.syntax( "delete" ,new caterwaul.syntax( "[]" ,new caterwaul.syntax( "L" ) ,new caterwaul.syntax( "_x" ) ) ) )'),qs1s:('new caterwaul.syntax( "[]" ,new caterwaul.syntax( "R" ) ,new caterwaul.syntax( "new" ,new caterwaul.syntax( "()" ,new caterwaul.syntax( "_f" ) ,new caterwaul.syntax( "_xs" ) ) ) )'),qs1t:('new caterwaul.syntax( "new" ,new caterwaul.syntax( "()" ,new caterwaul.syntax( "_f" ) ,new caterwaul.syntax( "[]" ,new caterwaul.syntax( "R" ) ,new caterwaul.syntax( "_xs" ) ) ) )'),qs1u:('new caterwaul.syntax( "[]" ,new caterwaul.syntax( "R" ) ,new caterwaul.syntax( "." ,new caterwaul.syntax( "_x" ) ,new caterwaul.syntax( "_y" ) ) )'),qs1v:('new caterwaul.syntax( "[]" ,new caterwaul.syntax( "H" ) ,new caterwaul.syntax( "." ,new caterwaul.syntax( "[]" ,new caterwaul.syntax( "R" ) ,new caterwaul.syntax( "_x" ) ) ,new caterwaul.syntax( "y" ) ) )'),qs1w:('new caterwaul.syntax( "[]" ,new caterwaul.syntax( "R" ) ,new caterwaul.syntax( "(" ,new caterwaul.syntax( "_x" ) ) )'),qs1x:('new caterwaul.syntax( "(" ,new caterwaul.syntax( "[]" ,new caterwaul.syntax( "R" ) ,new caterwaul.syntax( "_x" ) ) )'),qs1y:('new caterwaul.syntax( "[]" ,new caterwaul.syntax( "R" ) ,new caterwaul.syntax( "[]" ,new caterwaul.syntax( "_x" ) ,new caterwaul.syntax( "_y" ) ) )'),qs1z:('new caterwaul.syntax( "[]" ,new caterwaul.syntax( "H" ) ,new caterwaul.syntax( "[]" ,new caterwaul.syntax( "[]" ,new caterwaul.syntax( "R" ) ,new caterwaul.syntax( "_x" ) ) ,new caterwaul.syntax( "[]" ,new caterwaul.syntax( "R" ) ,new caterwaul.syntax( "_y" ) ) ) )'),qs20:('new caterwaul.syntax( "[]" ,new caterwaul.syntax( "R" ) ,new caterwaul.syntax( "[" ,new caterwaul.syntax( "_x" ) ) )'),qs21:('new caterwaul.syntax( "[]" ,new caterwaul.syntax( "H" ) ,new caterwaul.syntax( "[" ,new caterwaul.syntax( "[]" ,new caterwaul.syntax( "R" ) ,new caterwaul.syntax( "_x" ) ) ) )'),qs22:('new caterwaul.syntax( "[]" ,new caterwaul.syntax( "R" ) ,new caterwaul.syntax( "," ,new caterwaul.syntax( "_x" ) ,new caterwaul.syntax( "_y" ) ) )'),qs23:('new caterwaul.syntax( "," ,new caterwaul.syntax( "[]" ,new caterwaul.syntax( "R" ) ,new caterwaul.syntax( "_x" ) ) ,new caterwaul.syntax( "[]" ,new caterwaul.syntax( "R" ) ,new caterwaul.syntax( "_y" ) ) )'),qs24:('new caterwaul.syntax( "[]" ,new caterwaul.syntax( "R" ) ,new caterwaul.syntax( "{" ,new caterwaul.syntax( "_x" ) ) )'),qs25:('new caterwaul.syntax( "[]" ,new caterwaul.syntax( "H" ) ,new caterwaul.syntax( "{" ,new caterwaul.syntax( "[]" ,new caterwaul.syntax( "R" ) ,new caterwaul.syntax( "_x" ) ) ) )'),qs26:('new caterwaul.syntax( "[]" ,new caterwaul.syntax( "R" ) ,new caterwaul.syntax( "()" ,new caterwaul.syntax( "_x" ) ,new caterwaul.syntax( "_y" ) ) )'),qs27:('new caterwaul.syntax( "[]" ,new caterwaul.syntax( "H" ) ,new caterwaul.syntax( "()" ,new caterwaul.syntax( "[]" ,new caterwaul.syntax( "R" ) ,new caterwaul.syntax( "_x" ) ) ,new caterwaul.syntax( "[]" ,new caterwaul.syntax( "R" ) ,new caterwaul.syntax( "_y" ) ) ) )'),qs28:('new caterwaul.syntax( "[]" ,new caterwaul.syntax( "R" ) ,new caterwaul.syntax( "()" ,new caterwaul.syntax( "." ,new caterwaul.syntax( "_x" ) ,new caterwaul.syntax( "_y" ) ) ,new caterwaul.syntax( "_z" ) ) )'),qs29:('new caterwaul.syntax( "[]" ,new caterwaul.syntax( "H" ) ,new caterwaul.syntax( "()" ,new caterwaul.syntax( "." ,new caterwaul.syntax( "[]" ,new caterwaul.syntax( "R" ) ,new caterwaul.syntax( "_x" ) ) ,new caterwaul.syntax( "_y" ) ) ,new caterwaul.syntax( "[]" ,new caterwaul.syntax( "R" ) ,new caterwaul.syntax( "_z" ) ) ) )'),qs2a:('new caterwaul.syntax( "[]" ,new caterwaul.syntax( "R" ) ,new caterwaul.syntax( "()" ,new caterwaul.syntax( "[]" ,new caterwaul.syntax( "_x" ) ,new caterwaul.syntax( "_y" ) ) ,new caterwaul.syntax( "_z" ) ) )'),qs2b:('new caterwaul.syntax( "[]" ,new caterwaul.syntax( "H" ) ,new caterwaul.syntax( "()" ,new caterwaul.syntax( "[]" ,new caterwaul.syntax( "[]" ,new caterwaul.syntax( "R" ) ,new caterwaul.syntax( "_x" ) ) ,new caterwaul.syntax( "[]" ,new caterwaul.syntax( "R" ) ,new caterwaul.syntax( "_y" ) ) ) ,new caterwaul.syntax( "[]" ,new caterwaul.syntax( "R" ) ,new caterwaul.syntax( "_z" ) ) ) )'),qs2c:('new caterwaul.syntax( "[]" ,new caterwaul.syntax( "R" ) ,new caterwaul.syntax( "void" ,new caterwaul.syntax( "_x" ) ) )'),qs2d:('new caterwaul.syntax( "void" ,new caterwaul.syntax( "[]" ,new caterwaul.syntax( "R" ) ,new caterwaul.syntax( "_x" ) ) )'),qs2e:('new caterwaul.syntax( "[]" ,new caterwaul.syntax( "R" ) ,new caterwaul.syntax( "typeof" ,new caterwaul.syntax( "_x" ) ) )'),qs2f:('new caterwaul.syntax( "[]" ,new caterwaul.syntax( "H" ) ,new caterwaul.syntax( "," ,new caterwaul.syntax( "_x" ) ,new caterwaul.syntax( "typeof" ,new caterwaul.syntax( "[]" ,new caterwaul.syntax( "R" ) ,new caterwaul.syntax( "_x" ) ) ) ) )'),qs2g:('new caterwaul.syntax( "[]" ,new caterwaul.syntax( "R" ) ,new caterwaul.syntax( ":" ,new caterwaul.syntax( "_x" ) ,new caterwaul.syntax( "_y" ) ) )'),qs2h:('new caterwaul.syntax( ":" ,new caterwaul.syntax( "_x" ) ,new caterwaul.syntax( "[]" ,new caterwaul.syntax( "R" ) ,new caterwaul.syntax( "_y" ) ) )'),qs2i:('new caterwaul.syntax( "[]" ,new caterwaul.syntax( "R" ) ,new caterwaul.syntax( "typeof" ,new caterwaul.syntax( "_x@0" ) ) )'),qs2j:('new caterwaul.syntax( "[]" ,new caterwaul.syntax( "H" ) ,new caterwaul.syntax( "," ,new caterwaul.syntax( "_x" ) ,new caterwaul.syntax( "typeof" ,new caterwaul.syntax( "_x" ) ) ) )'),qs2k:('new caterwaul.syntax( "[]" ,new caterwaul.syntax( "R" ) ,new caterwaul.syntax( "u+" ,new caterwaul.syntax( "_x" ) ) )'),qs2l:('new caterwaul.syntax( "[]" ,new caterwaul.syntax( "H" ) ,new caterwaul.syntax( "u+" ,new caterwaul.syntax( "[]" ,new caterwaul.syntax( "R" ) ,new caterwaul.syntax( "_x" ) ) ) )'),qs2m:('new caterwaul.syntax( "[]" ,new caterwaul.syntax( "R" ) ,new caterwaul.syntax( "u~" ,new caterwaul.syntax( "_x" ) ) )'),qs2n:('new caterwaul.syntax( "[]" ,new caterwaul.syntax( "H" ) ,new caterwaul.syntax( "u~" ,new caterwaul.syntax( "[]" ,new caterwaul.syntax( "R" ) ,new caterwaul.syntax( "_x" ) ) ) )'),qs2o:('new caterwaul.syntax( "[]" ,new caterwaul.syntax( "R" ) ,new caterwaul.syntax( "u-" ,new caterwaul.syntax( "_x" ) ) )'),qs2p:('new caterwaul.syntax( "[]" ,new caterwaul.syntax( "H" ) ,new caterwaul.syntax( "u-" ,new caterwaul.syntax( "[]" ,new caterwaul.syntax( "R" ) ,new caterwaul.syntax( "_x" ) ) ) )'),qs2q:('new caterwaul.syntax( "[]" ,new caterwaul.syntax( "R" ) ,new caterwaul.syntax( "u!" ,new caterwaul.syntax( "_x" ) ) )'),qs2r:('new caterwaul.syntax( "[]" ,new caterwaul.syntax( "H" ) ,new caterwaul.syntax( "u!" ,new caterwaul.syntax( "[]" ,new caterwaul.syntax( "R" ) ,new caterwaul.syntax( "_x" ) ) ) )'),qs2s:('new caterwaul.syntax( "[]" ,new caterwaul.syntax( "R" ) ,new caterwaul.syntax( "u++" ,new caterwaul.syntax( "_x" ) ) )'),qs2t:('new caterwaul.syntax( "[]" ,new caterwaul.syntax( "H" ) ,new caterwaul.syntax( "u++" ,new caterwaul.syntax( "[]" ,new caterwaul.syntax( "L" ) ,new caterwaul.syntax( "_x" ) ) ) )'),qs2u:('new caterwaul.syntax( "[]" ,new caterwaul.syntax( "R" ) ,new caterwaul.syntax( "++" ,new caterwaul.syntax( "_x" ) ) )'),qs2v:('new caterwaul.syntax( "[]" ,new caterwaul.syntax( "H" ) ,new caterwaul.syntax( "++" ,new caterwaul.syntax( "[]" ,new caterwaul.syntax( "L" ) ,new caterwaul.syntax( "_x" ) ) ) )'),qs2w:('new caterwaul.syntax( "[]" ,new caterwaul.syntax( "R" ) ,new caterwaul.syntax( "u--" ,new caterwaul.syntax( "_x" ) ) )'),qs2x:('new caterwaul.syntax( "[]" ,new caterwaul.syntax( "H" ) ,new caterwaul.syntax( "u--" ,new caterwaul.syntax( "[]" ,new caterwaul.syntax( "L" ) ,new caterwaul.syntax( "_x" ) ) ) )'),qs2y:('new caterwaul.syntax( "[]" ,new caterwaul.syntax( "R" ) ,new caterwaul.syntax( "--" ,new caterwaul.syntax( "_x" ) ) )'),qs2z:('new caterwaul.syntax( "[]" ,new caterwaul.syntax( "H" ) ,new caterwaul.syntax( "--" ,new caterwaul.syntax( "[]" ,new caterwaul.syntax( "L" ) ,new caterwaul.syntax( "_x" ) ) ) )'),qs30:('new caterwaul.syntax( "[]" ,new caterwaul.syntax( "R" ) ,new caterwaul.syntax( "?" ,new caterwaul.syntax( "_x" ) ,new caterwaul.syntax( "_y" ) ,new caterwaul.syntax( "_z" ) ) )'),qs31:('new caterwaul.syntax( "[]" ,new caterwaul.syntax( "H" ) ,new caterwaul.syntax( "?" ,new caterwaul.syntax( "[]" ,new caterwaul.syntax( "R" ) ,new caterwaul.syntax( "_x" ) ) ,new caterwaul.syntax( "[]" ,new caterwaul.syntax( "R" ) ,new caterwaul.syntax( "_y" ) ) ,new caterwaul.syntax( "[]" ,new caterwaul.syntax( "R" ) ,new caterwaul.syntax( "_z" ) ) ) )'),qs32:('new caterwaul.syntax( "[]" ,new caterwaul.syntax( "R" ) ,new caterwaul.syntax( "function" ,new caterwaul.syntax( "(" ) ,new caterwaul.syntax( "{" ,new caterwaul.syntax( "_body" ) ) ) )'),qs33:('new caterwaul.syntax( "function" ,new caterwaul.syntax( "(" ) ,new caterwaul.syntax( "{" ,new caterwaul.syntax( ";" ,new caterwaul.syntax( "C" ) ,new caterwaul.syntax( "[]" ,new caterwaul.syntax( "S" ) ,new caterwaul.syntax( "_body" ) ) ) ) )'),qs34:('new caterwaul.syntax( "[]" ,new caterwaul.syntax( "R" ) ,new caterwaul.syntax( "function" ,new caterwaul.syntax( "(" ) ,new caterwaul.syntax( "{" ) ) )'),qs35:('new caterwaul.syntax( "function" ,new caterwaul.syntax( "(" ) ,new caterwaul.syntax( "{" ,new caterwaul.syntax( "C" ) ) )'),qs36:('new caterwaul.syntax( "[]" ,new caterwaul.syntax( "R" ) ,new caterwaul.syntax( "function" ,new caterwaul.syntax( "(" ,new caterwaul.syntax( "_xs" ) ) ,new caterwaul.syntax( "{" ,new caterwaul.syntax( "_body" ) ) ) )'),qs37:('new caterwaul.syntax( "function" ,new caterwaul.syntax( "(" ,new caterwaul.syntax( "_xs" ) ) ,new caterwaul.syntax( "{" ,new caterwaul.syntax( ";" ,new caterwaul.syntax( "C" ) ,new caterwaul.syntax( "[]" ,new caterwaul.syntax( "S" ) ,new caterwaul.syntax( "_body" ) ) ) ) )'),qs38:('new caterwaul.syntax( "[]" ,new caterwaul.syntax( "R" ) ,new caterwaul.syntax( "function" ,new caterwaul.syntax( "(" ,new caterwaul.syntax( "_xs" ) ) ,new caterwaul.syntax( "{" ) ) )'),qs39:('new caterwaul.syntax( "function" ,new caterwaul.syntax( "(" ,new caterwaul.syntax( "_xs" ) ) ,new caterwaul.syntax( "{" ,new caterwaul.syntax( "C" ) ) )'),qs3a:('new caterwaul.syntax( "[]" ,new caterwaul.syntax( "R" ) ,new caterwaul.syntax( "function" ,new caterwaul.syntax( "_f" ) ,new caterwaul.syntax( "(" ) ,new caterwaul.syntax( "{" ,new caterwaul.syntax( "_body" ) ) ) )'),qs3b:('new caterwaul.syntax( "function" ,new caterwaul.syntax( "_f" ) ,new caterwaul.syntax( "(" ) ,new caterwaul.syntax( "{" ,new caterwaul.syntax( ";" ,new caterwaul.syntax( "C" ) ,new caterwaul.syntax( "[]" ,new caterwaul.syntax( "S" ) ,new caterwaul.syntax( "_body" ) ) ) ) )'),qs3c:('new caterwaul.syntax( "[]" ,new caterwaul.syntax( "R" ) ,new caterwaul.syntax( "function" ,new caterwaul.syntax( "_f" ) ,new caterwaul.syntax( "(" ) ,new caterwaul.syntax( "{" ) ) )'),qs3d:('new caterwaul.syntax( "function" ,new caterwaul.syntax( "_f" ) ,new caterwaul.syntax( "(" ) ,new caterwaul.syntax( "{" ,new caterwaul.syntax( "C" ) ) )'),qs3e:('new caterwaul.syntax( "[]" ,new caterwaul.syntax( "R" ) ,new caterwaul.syntax( "function" ,new caterwaul.syntax( "_f" ) ,new caterwaul.syntax( "(" ,new caterwaul.syntax( "_xs" ) ) ,new caterwaul.syntax( "{" ,new caterwaul.syntax( "_body" ) ) ) )'),qs3f:('new caterwaul.syntax( "function" ,new caterwaul.syntax( "_f" ) ,new caterwaul.syntax( "(" ,new caterwaul.syntax( "_xs" ) ) ,new caterwaul.syntax( "{" ,new caterwaul.syntax( ";" ,new caterwaul.syntax( "C" ) ,new caterwaul.syntax( "[]" ,new caterwaul.syntax( "S" ) ,new caterwaul.syntax( "_body" ) ) ) ) )'),qs3g:('new caterwaul.syntax( "[]" ,new caterwaul.syntax( "R" ) ,new caterwaul.syntax( "function" ,new caterwaul.syntax( "_f" ) ,new caterwaul.syntax( "(" ,new caterwaul.syntax( "_xs" ) ) ,new caterwaul.syntax( "{" ) ) )'),qs3h:('new caterwaul.syntax( "function" ,new caterwaul.syntax( "_f" ) ,new caterwaul.syntax( "(" ,new caterwaul.syntax( "_xs" ) ) ,new caterwaul.syntax( "{" ,new caterwaul.syntax( "C" ) ) )'),qs3i:('new caterwaul.syntax( "(" ,new caterwaul.syntax( "," ,new caterwaul.syntax( "()" ,new caterwaul.syntax( "h" ) ,new caterwaul.syntax( "_x" ) ) ,new caterwaul.syntax( "()" ,new caterwaul.syntax( "h" ) ,new caterwaul.syntax( "," ,new caterwaul.syntax( "_x" ) ,new caterwaul.syntax( "(" ,new caterwaul.syntax( "_value" ) ) ) ) ) )'),qs3j:('new caterwaul.syntax( "()" ,new caterwaul.syntax( "h" ) ,new caterwaul.syntax( "," ,new caterwaul.syntax( "_x" ) ,new caterwaul.syntax( "(" ,new caterwaul.syntax( "_value" ) ) ) )'),qs3k:('new caterwaul.syntax( "[]" ,new caterwaul.syntax( "H" ) ,new caterwaul.syntax( "_x" ) )'),qs3l:('new caterwaul.syntax( "C" )'),qs3m:('new caterwaul.syntax( "null" )'),qs3n:('new caterwaul.syntax( "[]" ,new caterwaul.syntax( "S" ) ,new caterwaul.syntax( "_x" ) )'),qs3o:('new caterwaul.syntax( "()" ,new caterwaul.syntax( "eval" ) ,new caterwaul.syntax( "()" ,new caterwaul.syntax( "trace" ) ,new caterwaul.syntax( "[]" ,new caterwaul.syntax( "R" ) ,new caterwaul.syntax( "_x" ) ) ) )'),qs3p:('new caterwaul.syntax( "new" ,new caterwaul.syntax( "()" ,new caterwaul.syntax( "Function" ) ,new caterwaul.syntax( "[]" ,new caterwaul.syntax( "R" ) ,new caterwaul.syntax( "_xs" ) ) ) )'),qs3q:('new caterwaul.syntax( "()" ,new caterwaul.syntax( "trace" ) ,new caterwaul.syntax( "_x" ) )')};
return(br)}).call(this,new caterwaul.syntax("[]",new caterwaul.syntax("U"),new caterwaul.syntax("_x")),new caterwaul.syntax(""),new caterwaul.syntax("[]",new caterwaul.syntax("S"),new caterwaul.syntax("_x")),new caterwaul.syntax("[]",new caterwaul.syntax("R"),new caterwaul.syntax("_x")),new caterwaul.syntax("[]",new caterwaul.syntax("S"),new caterwaul.syntax("{",new caterwaul.syntax("_x"))),new caterwaul.syntax("{",new caterwaul.syntax("[]",new caterwaul.syntax("S"),new caterwaul.syntax("_x"))),new caterwaul.syntax("[]",new caterwaul.syntax("S"),new caterwaul.syntax("i;",new caterwaul.syntax("_x"),new caterwaul.syntax("_y"))),new caterwaul.syntax("i;",new caterwaul.syntax("[]",new caterwaul.syntax("S"),new caterwaul.syntax("_x")),new caterwaul.syntax("[]",new caterwaul.syntax("S"),new caterwaul.syntax("_y"))),new caterwaul.syntax("[]",new caterwaul.syntax("S"),new caterwaul.syntax(";",new caterwaul.syntax("_x"),new caterwaul.syntax("_y"))),new caterwaul.syntax(";",new caterwaul.syntax("[]",new caterwaul.syntax("S"),new caterwaul.syntax("_x")),new caterwaul.syntax("[]",new caterwaul.syntax("S"),new caterwaul.syntax("_y"))),new caterwaul.syntax("[]",new caterwaul.syntax("S"),new caterwaul.syntax("var",new caterwaul.syntax(",",new caterwaul.syntax("_vs"),new caterwaul.syntax("=",new caterwaul.syntax("_name"),new caterwaul.syntax("_value"))))),new caterwaul.syntax(";",new caterwaul.syntax("[]",new caterwaul.syntax("S"),new caterwaul.syntax("var",new caterwaul.syntax("_vs"))),new caterwaul.syntax("[]",new caterwaul.syntax("S"),new caterwaul.syntax("var",new caterwaul.syntax("=",new caterwaul.syntax("_name"),new caterwaul.syntax("_value"))))),new caterwaul.syntax("[]",new caterwaul.syntax("S"),new caterwaul.syntax("var",new caterwaul.syntax("=",new caterwaul.syntax("_name"),new caterwaul.syntax("_value")))),new caterwaul.syntax("var",new caterwaul.syntax("=",new caterwaul.syntax("[]",new caterwaul.syntax("L"),new caterwaul.syntax("_name")),new caterwaul.syntax("[]",new caterwaul.syntax("R"),new caterwaul.syntax("_value")))),new caterwaul.syntax("[]",new caterwaul.syntax("S"),new caterwaul.syntax("for",new caterwaul.syntax("(",new caterwaul.syntax(";",new caterwaul.syntax(";",new caterwaul.syntax("_x"),new caterwaul.syntax("_y")),new caterwaul.syntax("_z"))),new caterwaul.syntax("_body"))),new caterwaul.syntax("for",new caterwaul.syntax("(",new caterwaul.syntax(";",new caterwaul.syntax(";",new caterwaul.syntax("[]",new caterwaul.syntax("S"),new caterwaul.syntax("_x")),new caterwaul.syntax("[]",new caterwaul.syntax("R"),new caterwaul.syntax("_y"))),new caterwaul.syntax("[]",new caterwaul.syntax("R"),new caterwaul.syntax("_z")))),new caterwaul.syntax("[]",new caterwaul.syntax("S"),new caterwaul.syntax("_body"))),new caterwaul.syntax("[]",new caterwaul.syntax("S"),new caterwaul.syntax("for",new caterwaul.syntax("(",new caterwaul.syntax("var",new caterwaul.syntax("in",new caterwaul.syntax("_name"),new caterwaul.syntax("_o")))),new caterwaul.syntax("_body"))),new caterwaul.syntax("for",new caterwaul.syntax("(",new caterwaul.syntax("var",new caterwaul.syntax("in",new caterwaul.syntax("[]",new caterwaul.syntax("L"),new caterwaul.syntax("_name")),new caterwaul.syntax("[]",new caterwaul.syntax("R"),new caterwaul.syntax("_o"))))),new caterwaul.syntax("[]",new caterwaul.syntax("S"),new caterwaul.syntax("_body"))),new caterwaul.syntax("[]",new caterwaul.syntax("S"),new caterwaul.syntax("for",new caterwaul.syntax("(",new caterwaul.syntax("var",new caterwaul.syntax(",",new caterwaul.syntax("in",new caterwaul.syntax("_name"),new caterwaul.syntax("_1")),new caterwaul.syntax("_2")))),new caterwaul.syntax("_body"))),new caterwaul.syntax("for",new caterwaul.syntax("(",new caterwaul.syntax("var",new caterwaul.syntax(",",new caterwaul.syntax("in",new caterwaul.syntax("[]",new caterwaul.syntax("L"),new caterwaul.syntax("_name")),new caterwaul.syntax("[]",new caterwaul.syntax("R"),new caterwaul.syntax("_1"))),new caterwaul.syntax("[]",new caterwaul.syntax("R"),new caterwaul.syntax("_2"))))),new caterwaul.syntax("_body")),new caterwaul.syntax("[]",new caterwaul.syntax("S"),new caterwaul.syntax("for",new caterwaul.syntax("(",new caterwaul.syntax("var",new caterwaul.syntax("=",new caterwaul.syntax("in",new caterwaul.syntax("_name"),new caterwaul.syntax("_1")),new caterwaul.syntax("_2")))),new caterwaul.syntax("_body"))),new caterwaul.syntax("for",new caterwaul.syntax("(",new caterwaul.syntax("var",new caterwaul.syntax("=",new caterwaul.syntax("in",new caterwaul.syntax("[]",new caterwaul.syntax("L"),new caterwaul.syntax("_name")),new caterwaul.syntax("[]",new caterwaul.syntax("L"),new caterwaul.syntax("_1"))),new caterwaul.syntax("[]",new caterwaul.syntax("R"),new caterwaul.syntax("_2"))))),new caterwaul.syntax("_body")),new caterwaul.syntax("[]",new caterwaul.syntax("S"),new caterwaul.syntax("do",new caterwaul.syntax("_body"),new caterwaul.syntax("while",new caterwaul.syntax("(",new caterwaul.syntax("_cond"))))),new caterwaul.syntax("do",new caterwaul.syntax("S"),new caterwaul.syntax("[",new caterwaul.syntax("_body")),new caterwaul.syntax("while",new caterwaul.syntax("(",new caterwaul.syntax("[]",new caterwaul.syntax("R"),new caterwaul.syntax("_cond"))))),new caterwaul.syntax("[]",new caterwaul.syntax("S"),new caterwaul.syntax("while",new caterwaul.syntax("(",new caterwaul.syntax("_cond")),new caterwaul.syntax("_body"))),new caterwaul.syntax("while",new caterwaul.syntax("(",new caterwaul.syntax("[]",new caterwaul.syntax("R"),new caterwaul.syntax("_cond"))),new caterwaul.syntax("[]",new caterwaul.syntax("S"),new caterwaul.syntax("_body"))),new caterwaul.syntax("[]",new caterwaul.syntax("S"),new caterwaul.syntax("i;",new caterwaul.syntax("()",new caterwaul.syntax("with"),new caterwaul.syntax("_x")),new caterwaul.syntax("_body"))),new caterwaul.syntax("i;",new caterwaul.syntax("()",new caterwaul.syntax("with"),new caterwaul.syntax("[]",new caterwaul.syntax("R"),new caterwaul.syntax("_x"))),new caterwaul.syntax("[]",new caterwaul.syntax("S"),new caterwaul.syntax("_body"))),new caterwaul.syntax("[]",new caterwaul.syntax("S"),new caterwaul.syntax("if",new caterwaul.syntax("(",new caterwaul.syntax("_cond")),new caterwaul.syntax("_body"))),new caterwaul.syntax("if",new caterwaul.syntax("(",new caterwaul.syntax("[]",new caterwaul.syntax("R"),new caterwaul.syntax("_cond"))),new caterwaul.syntax("[]",new caterwaul.syntax("S"),new caterwaul.syntax("_body"))),new caterwaul.syntax("[]",new caterwaul.syntax("S"),new caterwaul.syntax("if",new caterwaul.syntax("(",new caterwaul.syntax("_cond")),new caterwaul.syntax("_body"),new caterwaul.syntax("else",new caterwaul.syntax("_else")))),new caterwaul.syntax("if",new caterwaul.syntax("(",new caterwaul.syntax("[]",new caterwaul.syntax("R"),new caterwaul.syntax("_cond"))),new caterwaul.syntax("[]",new caterwaul.syntax("S"),new caterwaul.syntax("_body")),new caterwaul.syntax("else",new caterwaul.syntax("[]",new caterwaul.syntax("S"),new caterwaul.syntax("_else")))),new caterwaul.syntax("[]",new caterwaul.syntax("S"),new caterwaul.syntax("try",new caterwaul.syntax("_x"),new caterwaul.syntax("catch",new caterwaul.syntax("_y"),new caterwaul.syntax("finally",new caterwaul.syntax("_z"))))),new caterwaul.syntax("try",new caterwaul.syntax("S"),new caterwaul.syntax("[",new caterwaul.syntax("_x")),new caterwaul.syntax("catch",new caterwaul.syntax("S"),new caterwaul.syntax("[",new caterwaul.syntax("_y")),new caterwaul.syntax("finally",new caterwaul.syntax("[]",new caterwaul.syntax("S"),new caterwaul.syntax("_z"))))),new caterwaul.syntax("[]",new caterwaul.syntax("S"),new caterwaul.syntax("try",new caterwaul.syntax("_x"),new caterwaul.syntax("catch",new caterwaul.syntax("_y")))),new caterwaul.syntax("try",new caterwaul.syntax("S"),new caterwaul.syntax("[",new caterwaul.syntax("_x")),new caterwaul.syntax("catch",new caterwaul.syntax("S"),new caterwaul.syntax("[",new caterwaul.syntax("_y")))),new caterwaul.syntax("[]",new caterwaul.syntax("S"),new caterwaul.syntax("try",new caterwaul.syntax("_x"),new caterwaul.syntax("finally",new caterwaul.syntax("_y")))),new caterwaul.syntax("i;",new caterwaul.syntax("try",new caterwaul.syntax("S"),new caterwaul.syntax("[",new caterwaul.syntax("_x"))),new caterwaul.syntax("finally",new caterwaul.syntax("[]",new caterwaul.syntax("S"),new caterwaul.syntax("_y")))),new caterwaul.syntax("[]",new caterwaul.syntax("S"),new caterwaul.syntax("throw",new caterwaul.syntax("_x"))),new caterwaul.syntax("throw",new caterwaul.syntax("[]",new caterwaul.syntax("R"),new caterwaul.syntax("_x"))),new caterwaul.syntax("[]",new caterwaul.syntax("S"),new caterwaul.syntax("return",new caterwaul.syntax("_x"))),new caterwaul.syntax("return",new caterwaul.syntax("[]",new caterwaul.syntax("R"),new caterwaul.syntax("_x"))),new caterwaul.syntax("[]",new caterwaul.syntax("S"),new caterwaul.syntax(";",new caterwaul.syntax("throw",new caterwaul.syntax("_x")),new caterwaul.syntax("_y"))),new caterwaul.syntax(";",new caterwaul.syntax("[]",new caterwaul.syntax("S"),new caterwaul.syntax("throw",new caterwaul.syntax("_x"))),new caterwaul.syntax("[]",new caterwaul.syntax("U"),new caterwaul.syntax("_y"))),new caterwaul.syntax("[]",new caterwaul.syntax("S"),new caterwaul.syntax(";",new caterwaul.syntax("return",new caterwaul.syntax("_x")),new caterwaul.syntax("_y"))),new caterwaul.syntax(";",new caterwaul.syntax("[]",new caterwaul.syntax("S"),new caterwaul.syntax("return",new caterwaul.syntax("_x"))),new caterwaul.syntax("[]",new caterwaul.syntax("U"),new caterwaul.syntax("_y"))),new caterwaul.syntax("[]",new caterwaul.syntax("S"),new caterwaul.syntax("i;",new caterwaul.syntax("()",new caterwaul.syntax("switch"),new caterwaul.syntax("_value")),new caterwaul.syntax("{",new caterwaul.syntax("_cases")))),new caterwaul.syntax("i;",new caterwaul.syntax("()",new caterwaul.syntax("switch"),new caterwaul.syntax("[]",new caterwaul.syntax("R"),new caterwaul.syntax("_value"))),new caterwaul.syntax("{",new caterwaul.syntax("[]",new caterwaul.syntax("S"),new caterwaul.syntax("_cases")))),new caterwaul.syntax("[]",new caterwaul.syntax("S"),new caterwaul.syntax(":",new caterwaul.syntax("_x"),new caterwaul.syntax("_y"))),new caterwaul.syntax(":",new caterwaul.syntax("_x"),new caterwaul.syntax("[]",new caterwaul.syntax("S"),new caterwaul.syntax("_y"))),new caterwaul.syntax("[]",new caterwaul.syntax("L"),new caterwaul.syntax("_x@0")),new caterwaul.syntax("_x"),new caterwaul.syntax("[]",new caterwaul.syntax("L"),new caterwaul.syntax("[]",new caterwaul.syntax("_x"),new caterwaul.syntax("_y"))),new caterwaul.syntax("[]",new caterwaul.syntax("[]",new caterwaul.syntax("R"),new caterwaul.syntax("_x")),new caterwaul.syntax("[]",new caterwaul.syntax("L"),new caterwaul.syntax("_y"))),new caterwaul.syntax("[]",new caterwaul.syntax("L"),new caterwaul.syntax(".",new caterwaul.syntax("_x"),new caterwaul.syntax("_y"))),new caterwaul.syntax(".",new caterwaul.syntax("[]",new caterwaul.syntax("R"),new caterwaul.syntax("_x")),new caterwaul.syntax("_y")),new caterwaul.syntax("[]",new caterwaul.syntax("L"),new caterwaul.syntax("()",new caterwaul.syntax("_x"),new caterwaul.syntax("_y"))),new caterwaul.syntax("[]",new caterwaul.syntax("U"),new caterwaul.syntax("()",new caterwaul.syntax("_x"),new caterwaul.syntax("_y"))),(function(c){var a,e,b,f,d;

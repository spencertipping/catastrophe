...
},ad=Object.prototype.hasOwnProperty,I=caterwaul.merge(caterwaul,{map:A,rmap:x,gensym:j,is_gensym:f,gensym_entropy:function(){return w}}),E=M(". new ++ -- u++ u-- u+ u- typeof void u~ u! ! * / % + - << >> >>> < > <= >= instanceof in == != === !== & ^ | && || ? = += -= *= /= %= &= |= ^= <<= >>= >>>= : , return throw case var const break continue else u; ;"),ah=function(aA){for(var az=0,ay=[false];
...
});return ax},add_bindings_to:function(ax){},add_expressions_to:function(ax){},resolve:function(){return this},reduce:function(){return this},contains:function(aA){var ax=aA(this);
if(ax){return ax}for(var az=0,ay=this.length;az<ay;++az){if(ax=this[az].contains(aA)){return ax}}},match:function(az,aA){az=az.constructor===String?I.parse(az):az;
aA||(aA={_:az});if(this.is_wildcard()&&(!this.leaf_nodes_only()||!this.length)){return aA[this.without_metadata()]=az,aA}else{if(this.length===az.length&&this.data===az.data){for(var ay=0,ax=this.length;
ay<ax;++ay){if(!this[ay].match(az[ay],aA)){return null}}return aA}}},toString:function(ay){var ax=[""];this.serialize(ax,ay||-1);return ax.join("")},structure:function(){if(this.length){return"("+['"'+this.data+'"'].concat(A(function(ax){return ax.structure()
...
});return aA>az?ax.as("("):ax},serialize:function(aA,aD){var ay=this.length,aC=this.data,ax=";\n",aB=aD-1,az=function(aE){if(K[aA[aA.length-1].charCodeAt(0)]===K[aE.charCodeAt(0)]){aA.push(" ",aE)
}else{aA.push(aE)}};if(ay&&aD===0){return az("...")}switch(ay){case 0:if(G(aw,aC)){return az(aC.replace(/^u/,""))}else{if(G(au,aC)){return az(aC),az(au[aC])}else{return az(aC)
}}case 1:if(G(P,aC)||G(aw,aC)){return az(aC.replace(/^u/,"")),this[0].serialize(aA,aB)}else{if(G(m,aC)){return this[0].serialize(aA,aB),az(aC)}else{if(G(au,aC)){return az(aC),this[0].serialize(aA,aB),az(au[aC])
}else{if(G(ab,aC)){return this[0].serialize(aA,aB)}else{return this[0].serialize(aA,aB),az(aC)}}}}case 2:if(G(Y,aC)){return this[0].serialize(aA,aB),az(aC.charAt(0)),this[1].serialize(aA,aB),az(aC.charAt(1))
}else{if(G(n,aC)){return az(aC),this[0].serialize(aA,aB),this[1].serialize(aA,aB)}else{if(G(X,aC)){return this[0].serialize(aA,aB),this[1].serialize(aA,aB)}else{if(aC===";"){return this[0].serialize(aA,aB),az(ax),this[1].serialize(aA,aB)
}else{return this[0].serialize(aA,aB),az(aC),this[1].serialize(aA,aB)}}}}default:if(G(y,aC)){return this[0].serialize(aA,aB),az(aC),this[1].precedence()>this.precedence()?(this[1].as("(").serialize(aA,aB),az(":"),this[2].serialize(aA,aB)):(this[1].serialize(aA,aB),az(":"),this[2].serialize(aA,aB))
}else{if(G(n,aC)){return this.accepts(this[2])&&!this[1].ends_with_block()?(az(aC),this[0].serialize(aA,aB),this[1].serialize(aA,aB),az(ax),this[2].serialize(aA,aB)):(az(aC),this[0].serialize(aA,aB),this[1].serialize(aA,aB),this[2].serialize(aA,aB))
}else{return this.unflatten().serialize(aA,aB)}}}}};I.ref_common=I.merge({},I.javascript_tree_type_methods,I.javascript_tree_metadata_methods,I.javascript_tree_serialization_methods,{replace:function(ax){var ay;
...
}else{this.e=ay,this.data=j(ax&&ax.constructor===String?ax:"e")}},I.ref_common,{add_expressions_to:function(ax){ax[this.data]=this.e}});I.metadata_node=I.syntax_subclass(function(ay,ax){if(ay instanceof this.constructor){this.metadata=ay.metadata,this.data=ay.data
}else{this.metadata=ay,this.data="@"+(ax||"")}},I.ref_common);I.opaque_tree=I.syntax_subclass(function(aA,ax){if(aA instanceof this.constructor){this.data=aA.data,this.expression_refs=aA.expression_refs
...
}},{resolve:function(){return this.expression_refs?I.late_bound_tree(new this.constructor(this.data),this.expression_refs):this},reduce:function(){return this.expression_refs?I.late_bound_tree(this.parse(),this.expression_refs):this.parse()
},guarded:function(){return this},serialize:function(ax){return ax.push(this.data),ax},parse:function(){return I.parse(this.data)}});I.syntax=ai(I.syntax_subclass(function(aC){if(aC instanceof this.constructor){this.data=aC.data,this.length=0
}else{this.data=aC&&aC.toString();this.length=0;for(var aA=1,ax=arguments.length,az;az=arguments[aA],aA<ax;++aA){for(var ay=0,aE=az.length,aB,aD;az instanceof Array?(aB=az[ay],ay<aE):(aB=az,!ay);
...
}});I.syntax.promote=function(ax){var ay=ax.constructor;return ay===String||ay===Number||ay===Boolean?new I.syntax(ax):ax};var ak=I.empty=new I.syntax("");I.parse=function(ax){if(ax==null){return ax
}if(ax.constructor===I.syntax){return ax}var aT=ax.toString().replace(/^\s*|\s*$/g,""),aC=0,a5=0,aQ=true,aP=false,a7=false,a6=false,aU=0,aR="",a2=0,aZ=aT.length,aK=function(a9){return aT.charCodeAt(a9)
},aI=[],aY=null,aG=null,aE=null,ay=A(function(){return[]},aq),aF=[],aS=[ak],a0=function(a9){return aS.push(a9),a9},aV=function(a9){return aG?aG._sibling(aG=a9):(aG=a9._append_to(aE)),a0(a9)
},az=this.syntax,aD=[],aH=[];if(aZ===0){return ak}while((aC=a2)<aZ){while(aj[a5=aK(a2)]&&a2<aZ){aC=++a2}aP=a6=a7=aR=false;if(am[a5]){aR=!!++a2;aQ=an[a5]}else{if(a5===l&&aK(a2+1)===r&&(a2+=2)){while(++a2<aZ&&aK(a2)!==l||aK(a2-1)!==r){}aR=!++a2
}else{if(a5===l&&aK(a2+1)===l){while(++a2<aZ&&!U[aK(a2)]){}aR=false}else{if(a5===aa){while(++a2<aZ&&!U[aK(a2)]){}aR=false}else{if(p[a5]&&(aU=a5)&&aQ&&!(aQ=!(aR=aT.charAt(a2)))){while(++a2<aZ&&(a5=aK(a2))!==aU||aP){aP=!aP&&a5===d
}while(++a2<aZ&&L[aK(a2)]){}aR=true}else{if(a5===i&&Z[aK(a2+1)]){while(++a2<aZ&&Z[aK(a2)]){}aQ=!(aR=true)}else{if(av[a5]&&(a5!==S||al[aK(a2+1)])){while(++a2<aZ&&(al[a5=aK(a2)]||(a7^(a7|=a5===S))||(a6^(a6|=h[a5]&&++a2)))){}while(a2<aZ&&al[aK(a2)]){++a2
}aQ=!(aR=true)}else{if(g[a5]&&(aR=aQ?"u":"",aQ=true)){while(a2<aZ&&g[aK(a2)]&&G(E,aR+aT.charAt(a2))){aR+=aT.charAt(a2++)}aQ=!G(B,aR)}else{while(++a2<aZ&&(K[a5=aK(a2)]||a5>127)){}aQ=G(E,aR=aT.substring(aC,a2))
}}}}}}}}if(a2===aC){throw new Error('Caterwaul lex error at "'+aT.substr(aC,40)+'" with leading context "'+aT.substr(aC-40,40)+'" (probably a Caterwaul bug)')}if(aR===false){continue
}aR=aR===true?aT.substring(aC,a2):aR==="u;"?";":aR;aR===aY?(aI.pop(),aY=aI[aI.length-1],aG=aG?aG.p:aE,aE=null):(G(au,aR)?(aI.push(aY=au[aR]),aE=aV(a0(new az(aR))),aD.push(aE),aG=null):aV(a0(new az(aR))),G(C,aR)&&ay[C[aR]].push(aG||aE));
aQ|=aR===")"&&aG.l&&G(n,aG.l.data)}for(var a2=0,aZ=ay.length,aO,a8;a8=ay[a2],aO=V[a2],a2<aZ;++a2){for(var a1=aO?0:a8.length-1,aN=a8.length,a3=aO?1:-1,aL,a4,aM;aO?a1<aN:a1>=0;
a1+=a3){if(G(ab,a4=(aL=a8[a1]).data)){if(a4===":"&&C[aL.r.data]>a2){aL._fold_l()}else{aL._fold_lr()}}else{if(G(R,a4)&&aL.l&&!((aM=aL.l.l)&&G(n,aM.data))&&(aL.l.data==="."||(aL.l.data==="function"&&aL.l.length===2)||!(G(E,aL.l.data)||G(J,aL.l.data)))){aF.push(aL.l._wrap(a0(new az(a4+au[a4]))).p._fold_r())
}else{if(G(Q,a4)){aL._fold_l()}else{if(G(P,a4)){aL._fold_r()}else{if(G(y,a4)){aL._fold_lr(),aH.push(aL)}else{if(G(n,a4)&&aL.r&&aL.r.data!==":"){for(var aW=0,aJ=n[a4];
aW<aJ&&aL.r&&!G(af,aL.r.data);++aW){aL._fold_r()}aL.r&&(aL.r.data===";"?aL.push(ak):aL._fold_r());if(G(s,a4)&&s[a4]===(aL.r&&aL.r.r&&aL.r.r.data)){aL._fold_r().pop()._fold_r()
}else{if(G(s,a4)&&s[a4]===(aL.r&&aL.r.data)){aL._fold_r()}}}else{if(G(aw,a4)){aL.r&&aL.r.data!==";"&&aL._fold_r()}}}}}}}}}for(var a2=aS.length-1,a8;a2>=0;--a2){(a8=aS[a2]).r&&a8._wrap(a0(new az("i;"))).p._fold_r()
}for(var a2=0,aZ=aF.length,a8,aA;a2<aZ;++a2){(aA=(a8=aF[a2])[1]=a8[1][0]||ak)&&(aA.p=a8)}for(var a2=0,aZ=aD.length,a8;a2<aZ;++a2){(a8=aD[a2]).length||a8.push(ak)
}for(var a2=0,aZ=aH.length,a8,aX,aB;a2<aZ;++a2){aX=(a8=aH[a2]).length,aB=a8[0],a8[0]=a8[aX-2],a8[1]=aB,a8[2]=a8[aX-1],a8.length=3}while(aG.p){aG=aG.p}for(var a2=aS.length-1,a8;
a2>=0;--a2){delete (a8=aS[a2]).p,delete a8.l,delete a8.r}return aG};var ae=I.parse("var _bindings; return(_expression)"),q=I.parse("_variable = _base._variable"),k=I.parse("undefined = void(0)"),D=I.parse("(function (_bindings) {var _result=(_body);_result_init;return(_result)}).call(this, _expressions)"),W=I.parse("_result.caterwaul_expression_ref_table = _expression_ref_table");
I.compile=function(aI,aC,aH){aH=I.merge({gensym_renaming:true,transparent_errors:false,unbound_closure:false,guard:true},aH);aI=I.late_bound_tree(aI,null,aH);if(aH.guard){aI=aI.guarded()
}var az=I.merge({},this._environment,aC,aI.bindings()),aE=[k],aG=j("base");for(var aB in az){if(ad.call(az,aB)&&aB!=="this"){aE.push(q.replace({_variable:aB,_base:aG}))
}}var aF=new this.syntax(",",aE).unflatten(),ax=ae.replace({_bindings:aF,_expression:aI});if(aH.gensym_renaming){var aA=this.gensym_rename_table(ax);for(var aB in az){ad.call(az,aB)&&(az[aA[aB]||aB]=az[aB])
}ax=ax.replace(aA);aG=aA[aG]}var ay=ax.toString(),aD=(function(){if(aH.transparent_errors){return new Function(aG,ay)}else{try{return new Function(aG,ay)}catch(aJ){throw new Error((aJ.message||aJ)+" while compiling "+ay)
}}})();return aH.unbound_closure?aD:aD.call(az["this"],az)};var H=I.parse("new caterwaul.syntax(_data)"),u=I.parse("new caterwaul.syntax(_data, _xs)");I.syntax_to_expression=function(ay){if(ay.length){for(var ax=new I.syntax(","),aA=0,az=ay.length;
...
caterwaul.module("std",function(a){a.js_all=function(){return this("js js_literals words seq")};a.all.push("js_all")});caterwaul.module("catastrophe",(function(a,aj,ah,af,ad,ab,Z,X,V,T,K,J,I,H,G,F,E,D,C,B,A,z,y,x,w,v,u,t,s,r,q,p,o,n,m,l,k,j,i,h,g,f,e,d,c,b,br,bq,bp,bo,bn,bm,bl,bk,bj,bi,bh,bg,bf,be,bc,ba,a8,a6,a4,a2,a0,aY,aW,aU,aT,bC,az,aS,bB,bA,bz,by,bx,bw,bv,bu,bt,bs,aR,aQ,aP,aO,aN,aM,aL,aK,aJ,aI,aH,aG,aF,aE,aD,aC,aB,aA,ay,ax,aw,av,au,at,ar,aq,bd,bb,a9,a7,a5,a3,a1,aZ,aX,aV,ap,ao,an,am,al,ak,ai,ag,ae,ac,aa,Y,W,U,S,R,Q,P,O,N,M){var L=(function(bD){(function(){var bJ=bD.context,bU={caterwaul:bD,local_scope:bN,immediate_scope:bR,closure_scope:bP},bO=function(bX,bV,bW){return bD.merge(bX,{xs:bV,pattern:bW},bQ)
},bQ={function_compiler:bD(":all"),promote:function(bV){return bV.constructor===Function?bV:this.compile_string(bV)},compile_string:function(bV){return this.function_compiler(bV,this.match_variables_for(this.pattern),{unbound_closure:true})
},match_variables_for:function(bV){return(function(b0){for(var bZ={},bY=0,bX=b0.length,bW;bY<bX;++bY){bW=b0[bY],bZ[bW[0]]=bW[1]}return bZ}).call(this,((function(bY){var bW,b0,bX,b1,bZ;
for(var bZ=new bY.constructor(),bX=0,b1=bY.length;bX<b1;++bX){bW=bY[bX],bZ.push(([bW.toString(),true]))}return bZ}).call(this,((["_","x","xs","xi","r","v","t"]).concat((bV.collect(function(bW){return bW.is_wildcard()
})))))))},select:function(bV){return bO(bV,this.xs,this.pattern)},iteration_context:function(bV){return bD.merge((this.pattern).match(this.xs[bV].tree())||{},{x:this.xs[bV]},(function(){var bZ=this.xs,b1=function(b2){return(function(b4){var b8,b7,b3,b9,b6;
for(var b9=b4.length,b3=b9-1,b8=b4[b3],b5;b3>=0;--b3){b8=b4[b3];if(b7=(b3<=bV?b8.tree()===b2:(b3=bV+1,0))){return b8}}return false}).call(this,bZ)},bX=bV,bW=function(b2){return b1(b2).value
},bY=bZ[bX].tree(),b0=function(b2){return b1(b2).time};return{xs:bZ,r:b1,xi:bX,v:bW,_:bY,t:b0}}).call(this))},map:function(bV){return(function(){var bW=this.promote(bV);
return(function(bZ){var bX,b1,bY,b2,b0;for(var b0=new bZ.constructor(),bY=0,b2=bZ.length;bY<b2;++bY){bX=bZ[bY],b0.push((bW(this.iteration_context(bX))))}return b0
}).call(this,this)}).call(this)},filter:function(bV){return(function(){var bW=this.promote(bV);return this.select((function(bZ){var bX,b1,bY,b2,b0;for(var b0=new bZ.constructor(),bY=0,b2=bZ.length;
bY<b2;++bY){bX=bZ[bY],(bW(this.iteration_context(bX)))&&b0.push(bX)}return b0}).call(this,this))}).call(this)},resolved:function(){return this.select((function(bX){var bV,bZ,bW,b0,bY;
for(var bY=new bX.constructor(),bW=0,b0=bX.length;bW<b0;++bW){bV=bX[bW],(this.xs[bV].resolved)&&bY.push(bV)}return bY}).call(this,this))},unresolved:function(){return this.select((function(bX){var bV,bZ,bW,b0,bY;
for(var bY=new bX.constructor(),bW=0,b0=bX.length;bW<b0;++bW){bV=bX[bW],(this.xs[bV].resolved)||bY.push(bV)}return bY}).call(this,this))},to_a:function(){return(function(bX){var bV,bZ,bW,b0,bY;
for(var bY=new bX.constructor(),bW=0,b0=bX.length;bW<b0;++bW){bV=bX[bW],bY.push((this.xs[bV]))}return bY}).call(this,this)},toString:function(bV){return(function(bW){return bW.join(", ")
}).call(this,((function(bY){var bW,b0,bX,b1,bZ;for(var bZ=new bY.constructor(),bX=0,b1=bY.length;bX<b1;++bX){bW=bY[bX],bZ.push((("["+(bW.tree().toString(bV||5))+"]")))
}return bZ}).call(this,this.to_a())))},first:function(bV){return this.select(this.slice(0,bV||1))},last:function(bV){return this.select(this.slice(this.length-(bV||1)))
},next:function(bV){var bV=bD.parse(bV);return this.select((function(bY){var bW,b0,bX,b1,bZ;for(var bZ=new bY.constructor(),bX=0,b1=bY.length;bX<b1;++bX){bW=bY[bX],bZ.push((bW-1))
}return bZ}).call(this,(function(bZ){var b2,b1,bY,bW,b0;for(var b0=new bZ.constructor(),bY=0,bW=bZ.length,bX;bY<bW;++bY){b2=bZ[bY],(bX=((function(b6){var b3,b8,b4,b9,b7;
for(var b3=b6[0],b4=0,b9=b6.length,b5;b4<b9;++b4){b3=b6[b4];if(b8=(b4>b2?(bV).match(b3.tree()):(b4=b2,0))){return b4+1}}return false}).call(this,this.xs)))&&b0.push(bX)
}return b0}).call(this,this)))},prev:function(bV){var bV=bD.parse(bV);return this.select((function(bY){var bW,b0,bX,b1,bZ;for(var bZ=new bY.constructor(),bX=0,b1=bY.length;
bX<b1;++bX){bW=bY[bX],bZ.push((bW-1))}return bZ}).call(this,(function(bZ){var b2,b1,bY,bW,b0;for(var b0=new bZ.constructor(),bY=0,bW=bZ.length,bX;bY<bW;++bY){b2=bZ[bY],(bX=((function(b6){var b3,b8,b4,b9,b7;
for(var b9=b6.length,b4=b9-1,b3=b6[b4],b5;b4>=0;--b4){b3=b6[b4];if(b8=(b4<b2?(bV).match(b3.tree()):(b4=b2,0))){return b4+1}}return false}).call(this,this.xs)))&&b0.push(bX)
}return b0}).call(this,this)))},interval:function(bV){return this.select((function(bY){var bW,b0,bX,b1,bZ;for(var bZ=new bY.constructor(),bX=0,b1=bY.length;bX<b1;
++bX){bW=bY[bX],bZ.push.apply(bZ,Array.prototype.slice.call(((function(b3,b2,b4){if((b2-b3)*b4<0||!b4){return[]}for(var b5=[],b6=b2-b3;b6>0?b3<=b2:b3>=b2;b3+=b4){b5.push(b3)
}return b5})((bW),(bV[bX]),(1)))))}return bZ}).call(this,this))},step:function(bV){return this.select((function(bY){var bW,b0,bX,b1,bZ;for(var bZ=new bY.constructor(),bX=0,b1=bY.length;
bX<b1;++bX){bW=bY[bX],(this.xs[bW])&&bZ.push(bW)}return bZ}).call(this,(function(bY){var bW,b0,bX,b1,bZ;for(var bZ=new bY.constructor(),bX=0,b1=bY.length;bX<b1;++bX){bW=bY[bX],bZ.push((bW+bV))
}return bZ}).call(this,this)))},adjacent:function(bV){return this.select((function(bY){var bW,b0,bX,b1,bZ;for(var bZ=new bY.constructor(),bX=0,b1=bY.length;bX<b1;
++bX){bW=bY[bX],bZ.push.apply(bZ,Array.prototype.slice.call(((function(b4){var b2,b6,b3,b7,b5;for(var b5=new b4.constructor(),b3=0,b7=b4.length;b3<b7;++b3){b2=b4[b3],(this.xs[b2])&&b5.push(b2)
}return b5}).call(this,(function(b3,b2,b4){if((b2-b3)*b4<0||!b4){return[]}for(var b5=[],b6=b2-b3;b6>0?b3<=b2:b3>=b2;b3+=b4){b5.push(b3)}return b5})((bW-bV),(bW+bV),(1))))))
}return bZ}).call(this,this))},to:function(bV){return(this).interval(this.next(bV))},from:function(bV){return(this.prev(bV)).interval(this)}},bG=function(bV){return(function(){var bZ=function(b9,b8){return arguments.length===2?b6(b9,b8):bX(b9)
},bX=function(b8){return b2.push(b5=bY(b8))},b6=function(b9,b8){return((b1(b9,b8),bV.trace_log_size&&b2.length>bV.trace_log_size<<1&&b7()),b8)},b7=function(){return b2.splice(b2.length-bV.trace_log_size,bV.trace_log_size)
},b3=function(b8){return(function(b9){return b5=(b5).find_sync(b8),b9}).call(this,(b5))},bY=function(b8){return bD.merge(new b0(b8),{pre_time:+new Date(),pre_id:++bW,bt:b5,resolved:false})
},b1=function(b9,b8){return(function(){var ca=bV.pre_trace?b3(b9):new b0(b9);return(bD.merge(ca,{value:b8,time:+new Date(),id:++bW,resolved:true}),!bV.pre_trace&&(b2).push(ca))
}).call(this)},b4={tree:function(){return bV.index[this.tree_id]},backtrace:function(){return(function(ca){var b8,cc,b9,cd,cb;for(var cb=[],b8=ca,b9=0,cc;cc=(b8);
++b9){cb.push(b8),b8=(b8.bt)}return cb}).call(this,this)},last_resolution:function(){return this.resolved||!this.bt?this:this.bt.last_resolution()},find_sync:function(b8){return this.tree_id===b8?this:this.bt.find_sync(b8)
},dt:function(){return this.time-this.pre_time},contains:function(b8){return !bV.pre_trace||b8.id>=this.pre_id&&b8.id<=this.id}},b0=(function(b8){return bD.merge(b8.prototype,b4),b8
}).call(this,(function(b8){return(this.tree_id=b8,this)})),bW=0,b5=null,b2=bV.trace_log;return bZ}).call(this)},bI=function(bW,bX,bV){return(function(){var bZ=bX.late_bound_tree(new bX.opaque_tree(bX.module(bV))),bY=bX.compile(bW(bZ),{caterwaul:bX},{gensym_renaming:false});
return bX.module(bV,bY)}).call(this)},bK=function(bV){return(function(){var b1={mocks:{},allow_mock_annotations:true,trace_native_eval_forms:true,environment:{},closures:true,hook_name:bD.gensym("hook"),hook:null,global:bJ,trace_log_size:1<<20,patterns:null,trace:true,pre_trace:true,hook_form:null,pre_hook_form:null},bY=bD.merge({},b1,bV),bW=function(){return b0.apply(this,arguments)
},bX=bY.trace_native_eval_forms?bL(bY.hook_name):{},bZ=bD.merge(bX,bY.mocks),b0=(function(b2){return b2.install_hook(),b2}).call(this,(bD.merge(bH(bD.merge(bY,{mocks:bZ,trace_log:[],index:{},patterns:bY.patterns&&(function(b6){var b2,b5,b7,b3,b4;
for(var b4=new b6.constructor(),b7=0,b3=b6.length;b7<b3;++b7){b2=b6[b7],b4.push((bD.parse(b2)))}return b4}).call(this,bY.patterns)})),(function(){var cc=bY,b7=function(){return this.find("_")
},b6=function(cd){var cd=bD.parse(cd);return bO((function(cg){var ce,ci,cf,cj,ch;for(var ch=new cg.constructor(),cf=0,cj=cg.length;cf<cj;++cf){ce=cg[cf],ch.push((ce-1))
}return ch}).call(this,(function(cg){var ce,ci,cf,ck,ch;for(var ch=new cg.constructor(),cf=0,ck=cg.length,cj;cf<ck;++cf){ce=cg[cf],(cj=((cd).match(ce.tree())&&cf+1))&&ch.push(cj)
}return ch}).call(this,b5)),b5,cd)},b2=function(ce,cd){return bI(this,ce,cd)},b8=function(cd){return(cd.toString()).indexOf(bD.gensym_entropy())>-1},ca=function(ce,cd){return cd?ce.call(cc.hook,cd):ce.call(cc.hook)
},b5=cc.trace_log,b9=cc.hook?cc.hook:cc.hook=bG(cc),cb=cc.global&&(b9.trace_eval?b9.trace_eval:b9.trace_eval=bE(bW)),b4=function(){return cc.global&&(cc.global[cc.hook_name]=b9)
},b3=function(){return cc.global&&delete cc.global[cc.hook_name]};return{options:cc,unresolved:b7,find:b6,trace_module:b2,is_traced:b8,closure_state:ca,trace_log:b5,hook:b9,trace_eval:cb,install_hook:b4,remove_hook:b3}
}).call(this))));return b0}).call(this)},bF=function(bV){return(function(){var bW=function(bY,bX){return(function(){var cc=function(){return(((((b3).concat((ch))).concat((b8))).concat((b2))).concat((b0))).concat((b5))
},cb=function(){return(function(){var ck=bV.allow_mock_annotations?bX:function(cl){return cl},cj=function(cl){return cl.constructor===Function?cl:ck(bD.parse(cl))
};return(function(cn){var cl,cp,cm,cq,co;for(var co=new cn.constructor(),cm=0,cq=cn.length;cm<cq;++cm){cl=cn[cm],co.push((b6(bX(bD.parse(cl[0])),cj(cj(cl[1])))))
}return co}).call(this,(function(cn){var cm=[];for(var cl in cn){Object.prototype.hasOwnProperty.call(cn,cl)&&cm.push([cl,cn[cl]])}return cm}).call(this,(bV.mocks)))
}).call(this)},b9=function(cj){return(function(){var ck=function(cl){return(function(cm){return cm&&this(cm)}).call(this,(cj.call(this,cl)))};return ck}).call(this)
},b6=function(cj,ck){return b9((a).match(ck)?(function(){var cl=bY(cj,ck);return function(cm){return(function(cn){return cn&&cm[1].id(),cn[1].id=cm[1].id,cn}).call(this,(cl.call(this,cm)))
}}).call(this):bY(cj,ck))},b3=[b6(aj,ah)],ch=[b6(af,ad),b6(ab,Z),b6(X,V),b6(T,K),b6(J,I),b6(H,G),b6(F,E),b6(D,C),b6(B,A),b6(z,y),b6(x,w),b6(v,u),b6(t,s),b6(r,q),b6(p,o),b6(n,m),b6(l,k),b6(j,i),b6(h,g),b6(f,e),b6(d,c),b6(b,br),b6(bq,bp),b6(bo,bn),b6(bm,bl),b6(bk,bj),b6(bi,bh),b6(bg,bf),b6(be,bc),b6(ba,a8)],b8=[b6(a6,a4),b6(a2,a0),b6(aY,aW),b6(aU,aT)],b2=(function(){var ck=bC,cj=az;
return(([b6(aS,bB),b6(bA,bz),b6(by,bx),b6(bw,bv),b6(bu,bt),b6(bs,aR),b6(aQ,aP),b6(aO,aN),b6(aM,aL),b6(aK,aJ),b6(aI,aH),b6(aG,aF),b6(aE,aD),b6(aC,aB),b6(aA,ay),b6(ax,aw),b6(av,au),b6(at,ar),b6(aq,bd),b6(bb,a9),b6(a7,a5),b6(a3,a1),b6(aZ,aX),b6(aV,ap),b6(ao,an)]).concat(((function(cn){var cl,cp,cm,cq,co;
for(var co=new cn.constructor(),cm=0,cq=cn.length;cm<cq;++cm){cl=cn[cm],co.push((b6(cl[0],cl[1])))}return co}).call(this,((ck).concat((cj))))))).concat(([b6(am,al),b6(ak,ai),b6(ag,ae)]))
}).call(this),cg=new bD.syntax(bV.hook_name),cf=(bD.parse(bV.pre_hook_form)||ac).replace({hook:cg}),b7=(bD.parse(bV.hook_form)||aa).replace({hook:cg}),ca=function(cj){return cj.without_markers?cj.without_markers:cj.without_markers=((cj.data==="[]"&&bD.is_gensym(cj[0].data)?cj[1]:cj)).map(ca)
},b4=function(cj){return cj.is_constant()||bV.patterns&&(function(cn){var ck,cp,cl,cq,co;for(var ck=cn[0],cl=0,cq=cn.length,cm;cl<cq;++cl){ck=cn[cl];if(cm=((ck).match(ca(cj)))){return false
}}return true}).call(this,bV.patterns)?cj:((bV.pre_trace?cf:b7)).replace({_tree:bD.syntax.from_string(cj.id()),_value:cj})},b0=[b6(Y,(bV.trace?function(cj){return b4(cj._x)
}:W))],cd=U,ce=function(cj){return(S).replace({_name:bD.syntax.from_string(cj),_v:cj})},b1=function(cj){return bD.syntax.from_object((function(co){for(var cn={},cm=0,cl=co.length,ck;
cm<cl;++cm){ck=co[cm],cn[ck[0]]=ck[1]}return cn}).call(this,((function(cm){var ck,co,cl,cp,cn;for(var cn=new cm.constructor(),cl=0,cp=cm.length;cl<cp;++cl){ck=cm[cl],cn.push(([ck,new bD.syntax(ck)]))
}return cn}).call(this,(function(cl){var cm=[];for(var ck in cl){Object.prototype.hasOwnProperty.call(cl,ck)&&cm.push(ck)}return cm}).call(this,(cj))))))},bZ=function(cj){return(function(ck){return ck.length?ck:bD.empty
}).call(this,(new bD.syntax(",",(function(co){var ck,cn,cp,cl,cm;for(var cm=new co.constructor(),cp=0,cl=co.length;cp<cl;++cp){ck=co[cp],cm.push((ce(ck)))}return cm
}).call(this,(function(cl){var cm=[];for(var ck in cl){Object.prototype.hasOwnProperty.call(cl,ck)&&cm.push(ck)}return cm}).call(this,(cj)))).unflatten()))},ci=function(cj){return(cd).replace({_hook:bV.hook_name,_get:b1(cj),_set:bZ(cj)})
},b5=[b6(R,(bV.closures?function(cj){return ci(cj._scope.metadata)}:Q))];return(cc()).concat((cb()))}).call(this)};return(function(){var bX=bD.grammar(["L","U","R","C","H","S"],{initial:"S[_x]",fix:false},bW),bY=function(b1){return(b1).reach(function(b2){return bV.index[b2.id()]=b2
})},b0=function(b1){return(b1).rmap(function(b2){return(function(b3){return b3!==b2?b0(b3):b2}).call(this,(b2.reduce()))})},bZ=function(b1){return bX({_x:bS(bY(b0(b1)),bV)})
};return bZ}).call(this)}).call(this)},bM=function(bW,bV){return(bW(bV),(bV).each(function(bX){return !(bX.data==="function")&&bM(bW,bX)}))},bR=function(bV){return(function(){var bW=function(bY){return bY.is_identifier()&&(bX[bY.data]=bY)
},bX={};return(bM(bW,bD.parse(bV)),bX)}).call(this)},bN=function(bV){return(function(){var bW=function(b0){return b0.data==="var"&&bY(b0[0].flatten(","))},bY=function(b0){return(function(b1){return b1.data==="in"?bX[b1[0].data]=b1:bZ(b0)
}).call(this,(b0[0].flatten("=")[0]))},bZ=function(b0){return(function(b3){var b1,b5,b2,b6,b4;for(var b2=0,b6=b3.length;b2<b6;++b2){b1=b3[b2],(bX[b1.flatten("=")[0].data]=b1)
}return b3}).call(this,b0)},bX=(function(b4){for(var b3={},b2=0,b1=b4.length,b0;b2<b1;++b2){b0=b4[b2],b3[b0[0]]=b0[1]}return b3}).call(this,((function(b2){var b0,b4,b1,b5,b3;
for(var b3=new b2.constructor(),b1=0,b5=b2.length;b1<b5;++b1){b0=b2[b1],b3.push(([b0.data,b0]))}return b3}).call(this,(function(b2){var b0,b4,b1,b5,b3;for(var b3=new b2.constructor(),b1=0,b5=b2.length;
b1<b5;++b1){b0=b2[b1],(b0.data.length)&&b3.push(b0)}return b3}).call(this,Array.prototype.slice.call((bV[0][0].flatten(",")))))));return(bM(bW,bD.parse(bV)),bX)}).call(this)
},bP=function(bV){return(function(){var bW=bN(bV);return(function(bZ){var bX,b1,bY,b2,b0;var b0=new bZ.constructor();for(var bX in bZ){if(Object.prototype.hasOwnProperty.call(bZ,bX)&&!(Object.prototype.hasOwnProperty.call(bW,bX))){b0[bX]=bZ[bX]
}}return b0}).call(this,bR(bV))}).call(this)},bT=function(bV){return function(bW){return(function(bX){return bX[0][0]=new bD.syntax(";",bX[0][0]||bD.empty,new bD.metadata_node(bV.closures&&bP(bW),"scope")),bX
}).call(this,(bW))}},bS=function(bW,bV){return(function(){var bX=bT(bV);return((function(b2){var bY,b1,b3,bZ,b0;for(var b3=0,bZ=b2.length;b3<bZ;++b3){bY=b2[b3],(bX(bY))
}return b2}).call(this,bW.collect(function(bY){return bY.data==="function"})),bW)}).call(this)},bE=function(bV){return function(bW){return bV(bD.parse(bW)).toString()
}},bL=function(bV){return(function(){var bW=(P).replace({hook:bV});return(function(bZ){var bX,b1,bY,b3,b0;var b0=new bZ.constructor();for(var b2 in bZ){if(Object.prototype.hasOwnProperty.call(bZ,b2)){bX=bZ[b2],b0[b2]=((bX).replace({trace:bW}))
}}return b0}).call(this,{"R[eval(_x)]":O,"R[new Function(_x)]":N,"R[new Function(_xs, _x)]":M})}).call(this)},bH=function(bV){return(function(){var bY=bF(bV),bX=bD(bY),bW=function(bZ,b0){return bZ.constructor===String?this.find.apply(this,arguments):bX(bZ,bD.merge({},bV.environment,b0),{transparent_errors:false,gensym_renaming:false})
};return bW}).call(this)};return bD.merge((catastrophe=bK),bU)}).call(this)});L.caterwaul_expression_ref_table={qs:('new caterwaul.syntax( "[]" ,new caterwaul.syntax( "H" ) ,new caterwaul.syntax( "_x" ) )'),qs1:('new caterwaul.syntax( "[]" ,new caterwaul.syntax( "U" ) ,new caterwaul.syntax( "_x" ) )'),qs2:('new caterwaul.syntax( "" )'),qs3:('new caterwaul.syntax( "[]" ,new caterwaul.syntax( "S" ) ,new caterwaul.syntax( "_x" ) )'),qs4:('new caterwaul.syntax( "[]" ,new caterwaul.syntax( "R" ) ,new caterwaul.syntax( "_x" ) )'),qs5:('new caterwaul.syntax( "[]" ,new caterwaul.syntax( "S" ) ,new caterwaul.syntax( "{" ,new caterwaul.syntax( "_x" ) ) )'),qs6:('new caterwaul.syntax( "{" ,new caterwaul.syntax( "[]" ,new caterwaul.syntax( "S" ) ,new caterwaul.syntax( "_x" ) ) )'),qs7:('new caterwaul.syntax( "[]" ,new caterwaul.syntax( "S" ) ,new caterwaul.syntax( "i;" ,new caterwaul.syntax( "_x" ) ,new caterwaul.syntax( "_y" ) ) )'),qs8:('new caterwaul.syntax( "i;" ,new caterwaul.syntax( "[]" ,new caterwaul.syntax( "S" ) ,new caterwaul.syntax( "_x" ) ) ,new caterwaul.syntax( "[]" ,new caterwaul.syntax( "S" ) ,new caterwaul.syntax( "_y" ) ) )'),qs9:('new caterwaul.syntax( "[]" ,new caterwaul.syntax( "S" ) ,new caterwaul.syntax( ";" ,new caterwaul.syntax( "_x" ) ,new caterwaul.syntax( "_y" ) ) )'),qsa:('new caterwaul.syntax( ";" ,new caterwaul.syntax( "[]" ,new caterwaul.syntax( "S" ) ,new caterwaul.syntax( "_x" ) ) ,new caterwaul.syntax( "[]" ,new caterwaul.syntax( "S" ) ,new caterwaul.syntax( "_y" ) ) )'),qsb:('new caterwaul.syntax( "[]" ,new caterwaul.syntax( "S" ) ,new caterwaul.syntax( "var" ,new caterwaul.syntax( "," ,new caterwaul.syntax( "_vs" ) ,new caterwaul.syntax( "_x" ) ) ) )'),qsc:('new caterwaul.syntax( "," ,new caterwaul.syntax( "[]" ,new caterwaul.syntax( "S" ) ,new caterwaul.syntax( "var" ,new caterwaul.syntax( "_vs" ) ) ) ,new caterwaul.syntax( "[]" ,new caterwaul.syntax( "R" ) ,new caterwaul.syntax( "_x" ) ) )'),qsd:('new caterwaul.syntax( "[]" ,new caterwaul.syntax( "S" ) ,new caterwaul.syntax( "if" ,new caterwaul.syntax( "(" ,new caterwaul.syntax( "_cond" ) ) ,new caterwaul.syntax( "_body" ) ) )'),qse:('new caterwaul.syntax( "if" ,new caterwaul.syntax( "(" ,new caterwaul.syntax( "[]" ,new caterwaul.syntax( "R" ) ,new caterwaul.syntax( "_cond" ) ) ) ,new caterwaul.syntax( "[]" ,new caterwaul.syntax( "S" ) ,new caterwaul.syntax( "_body" ) ) )'),qsf:('new caterwaul.syntax( "[]" ,new caterwaul.syntax( "S" ) ,new caterwaul.syntax( "var" ,new caterwaul.syntax( "," ,new caterwaul.syntax( "_vs" ) ,new caterwaul.syntax( "_name@0" ) ) ) )'),qsg:('new caterwaul.syntax( ";" ,new caterwaul.syntax( "[]" ,new caterwaul.syntax( "S" ) ,new caterwaul.syntax( "var" ,new caterwaul.syntax( "_vs" ) ) ) ,new caterwaul.syntax( "[]" ,new caterwaul.syntax( "S" ) ,new caterwaul.syntax( "var" ,new caterwaul.syntax( "_name" ) ) ) )'),qsh:('new caterwaul.syntax( "[]" ,new caterwaul.syntax( "S" ) ,new caterwaul.syntax( "if" ,new caterwaul.syntax( "(" ,new caterwaul.syntax( "_cond" ) ) ,new caterwaul.syntax( "_body" ) ,new caterwaul.syntax( "else" ,new caterwaul.syntax( "_else" ) ) ) )'),qsi:('new caterwaul.syntax( "if" ,new caterwaul.syntax( "(" ,new caterwaul.syntax( "[]" ,new caterwaul.syntax( "R" ) ,new caterwaul.syntax( "_cond" ) ) ) ,new caterwaul.syntax( "[]" ,new caterwaul.syntax( "S" ) ,new caterwaul.syntax( "_body" ) ) ,new caterwaul.syntax( "else" ,new caterwaul.syntax( "[]" ,new caterwaul.syntax( "S" ) ,new caterwaul.syntax( "_else" ) ) ) )'),qsj:('new caterwaul.syntax( "[]" ,new caterwaul.syntax( "S" ) ,new caterwaul.syntax( "var" ,new caterwaul.syntax( "," ,new caterwaul.syntax( "_vs" ) ,new caterwaul.syntax( "=" ,new caterwaul.syntax( "_name" ) ,new caterwaul.syntax( "_value" ) ) ) ) )'),qsk:('new caterwaul.syntax( ";" ,new caterwaul.syntax( "[]" ,new caterwaul.syntax( "S" ) ,new caterwaul.syntax( "var" ,new caterwaul.syntax( "_vs" ) ) ) ,new caterwaul.syntax( "[]" ,new caterwaul.syntax( "S" ) ,new caterwaul.syntax( "var" ,new caterwaul.syntax( "=" ,new caterwaul.syntax( "_name" ) ,new caterwaul.syntax( "_value" ) ) ) ) )'),qsl:('new caterwaul.syntax( "[]" ,new caterwaul.syntax( "S" ) ,new caterwaul.syntax( "var" ,new caterwaul.syntax( "_name@0" ) ) )'),qsm:('new caterwaul.syntax( "var" ,new caterwaul.syntax( "_name" ) )'),qsn:('new caterwaul.syntax( "[]" ,new caterwaul.syntax( "S" ) ,new caterwaul.syntax( "throw" ,new caterwaul.syntax( "_x" ) ) )'),qso:('new caterwaul.syntax( "throw" ,new caterwaul.syntax( "[]" ,new caterwaul.syntax( "R" ) ,new caterwaul.syntax( "_x" ) ) )'),qsp:('new caterwaul.syntax( "[]" ,new caterwaul.syntax( "S" ) ,new caterwaul.syntax( "var" ,new caterwaul.syntax( "=" ,new caterwaul.syntax( "_name" ) ,new caterwaul.syntax( "_value" ) ) ) )'),qsq:('new caterwaul.syntax( "var" ,new caterwaul.syntax( "=" ,new caterwaul.syntax( "_name" ) ,new caterwaul.syntax( "[]" ,new caterwaul.syntax( "R" ) ,new caterwaul.syntax( "_value" ) ) ) )'),qsr:('new caterwaul.syntax( "[]" ,new caterwaul.syntax( "S" ) ,new caterwaul.syntax( "var" ,new caterwaul.syntax( "in" ,new caterwaul.syntax( "_name" ) ,new caterwaul.syntax( "_value" ) ) ) )'),qss:('new caterwaul.syntax( "var" ,new caterwaul.syntax( "in" ,new caterwaul.syntax( "_name" ) ,new caterwaul.syntax( "[]" ,new caterwaul.syntax( "R" ) ,new caterwaul.syntax( "_value" ) ) ) )'),qst:('new caterwaul.syntax( "[]" ,new caterwaul.syntax( "S" ) ,new caterwaul.syntax( "for" ,new caterwaul.syntax( "(" ,new caterwaul.syntax( "_x" ) ) ,new caterwaul.syntax( "_body" ) ) )'),qsu:('new caterwaul.syntax( "for" ,new caterwaul.syntax( "(" ,new caterwaul.syntax( "[]" ,new caterwaul.syntax( "S" ) ,new caterwaul.syntax( "_x" ) ) ) ,new caterwaul.syntax( "[]" ,new caterwaul.syntax( "S" ) ,new caterwaul.syntax( "_body" ) ) )'),qsv:('new caterwaul.syntax( "[]" ,new caterwaul.syntax( "S" ) ,new caterwaul.syntax( "do" ,new caterwaul.syntax( "_body" ) ,new caterwaul.syntax( "while" ,new caterwaul.syntax( "(" ,new caterwaul.syntax( "_cond" ) ) ) ) )'),qsw:('new caterwaul.syntax( "do" ,new caterwaul.syntax( "S" ) ,new caterwaul.syntax( "[" ,new caterwaul.syntax( "_body" ) ) ,new caterwaul.syntax( "while" ,new caterwaul.syntax( "(" ,new caterwaul.syntax( "[]" ,new caterwaul.syntax( "R" ) ,new caterwaul.syntax( "_cond" ) ) ) ) )'),qsx:('new caterwaul.syntax( "[]" ,new caterwaul.syntax( "S" ) ,new caterwaul.syntax( "while" ,new caterwaul.syntax( "(" ,new caterwaul.syntax( "_cond" ) ) ,new caterwaul.syntax( "_body" ) ) )'),qsy:('new caterwaul.syntax( "while" ,new caterwaul.syntax( "(" ,new caterwaul.syntax( "[]" ,new caterwaul.syntax( "R" ) ,new caterwaul.syntax( "_cond" ) ) ) ,new caterwaul.syntax( "[]" ,new caterwaul.syntax( "S" ) ,new caterwaul.syntax( "_body" ) ) )'),qsz:('new caterwaul.syntax( "[]" ,new caterwaul.syntax( "S" ) ,new caterwaul.syntax( "with" ,new caterwaul.syntax( "(" ,new caterwaul.syntax( "_x" ) ) ,new caterwaul.syntax( "_body" ) ) )'),qs10:('new caterwaul.syntax( "with" ,new caterwaul.syntax( "(" ,new caterwaul.syntax( "[]" ,new caterwaul.syntax( "R" ) ,new caterwaul.syntax( "_x" ) ) ) ,new caterwaul.syntax( "[]" ,new caterwaul.syntax( "S" ) ,new caterwaul.syntax( "_body" ) ) )'),qs11:('new caterwaul.syntax( "[]" ,new caterwaul.syntax( "S" ) ,new caterwaul.syntax( "continue" ,new caterwaul.syntax( "_label" ) ) )'),qs12:('new caterwaul.syntax( "continue" ,new caterwaul.syntax( "_label" ) )'),qs13:('new caterwaul.syntax( "[]" ,new caterwaul.syntax( "S" ) ,new caterwaul.syntax( "continue" ) )'),qs14:('new caterwaul.syntax( "continue" )'),qs15:('new caterwaul.syntax( "[]" ,new caterwaul.syntax( "S" ) ,new caterwaul.syntax( "break" ,new caterwaul.syntax( "_label" ) ) )'),qs16:('new caterwaul.syntax( "break" ,new caterwaul.syntax( "_label" ) )'),qs17:('new caterwaul.syntax( "[]" ,new caterwaul.syntax( "S" ) ,new caterwaul.syntax( "break" ) )'),qs18:('new caterwaul.syntax( "break" )'),qs19:('new caterwaul.syntax( "[]" ,new caterwaul.syntax( "S" ) ,new caterwaul.syntax( "return" ,new caterwaul.syntax( "_x" ) ) )'),qs1a:('new caterwaul.syntax( "return" ,new caterwaul.syntax( "[]" ,new caterwaul.syntax( "R" ) ,new caterwaul.syntax( "_x" ) ) )'),qs1b:('new caterwaul.syntax( "[]" ,new caterwaul.syntax( "S" ) ,new caterwaul.syntax( "return" ) )'),qs1c:('new caterwaul.syntax( "return" )'),qs1d:('new caterwaul.syntax( "[]" ,new caterwaul.syntax( "S" ) ,new caterwaul.syntax( "for" ,new caterwaul.syntax( "(" ,new caterwaul.syntax( "in" ,new caterwaul.syntax( "_name" ) ,new caterwaul.syntax( "_v" ) ) ) ,new caterwaul.syntax( "_body" ) ) )'),qs1e:('new caterwaul.syntax( "for" ,new caterwaul.syntax( "(" ,new caterwaul.syntax( "in" ,new caterwaul.syntax( "_name" ) ,new caterwaul.syntax( "[]" ,new caterwaul.syntax( "R" ) ,new caterwaul.syntax( "_v" ) ) ) ) ,new caterwaul.syntax( "_body" ) )'),qs1f:('new caterwaul.syntax( "[]" ,new caterwaul.syntax( "S" ) ,new caterwaul.syntax( "i;" ,new caterwaul.syntax( "()" ,new caterwaul.syntax( "switch" ) ,new caterwaul.syntax( "_value" ) ) ,new caterwaul.syntax( "{" ,new caterwaul.syntax( "_cases" ) ) ) )'),qs1g:('new caterwaul.syntax( "i;" ,new caterwaul.syntax( "()" ,new caterwaul.syntax( "switch" ) ,new caterwaul.syntax( "[]" ,new caterwaul.syntax( "R" ) ,new caterwaul.syntax( "_value" ) ) ) ,new caterwaul.syntax( "{" ,new caterwaul.syntax( "[]" ,new caterwaul.syntax( "S" ) ,new caterwaul.syntax( "_cases" ) ) ) )'),qs1h:('new caterwaul.syntax( "[]" ,new caterwaul.syntax( "S" ) ,new caterwaul.syntax( ":" ,new caterwaul.syntax( "_x" ) ,new caterwaul.syntax( "_y" ) ) )'),qs1i:('new caterwaul.syntax( ":" ,new caterwaul.syntax( "_x" ) ,new caterwaul.syntax( "[]" ,new caterwaul.syntax( "S" ) ,new caterwaul.syntax( "_y" ) ) )'),qs1j:('new caterwaul.syntax( "[]" ,new caterwaul.syntax( "S" ) ,new caterwaul.syntax( "function" ,new caterwaul.syntax( "_f" ) ,new caterwaul.syntax( "(" ,new caterwaul.syntax( ";" ,new caterwaul.syntax( "_xs" ) ,new caterwaul.syntax( "_scope" ) ) ) ,new caterwaul.syntax( "{" ,new caterwaul.syntax( "_body" ) ) ) )'),qs1k:('new caterwaul.syntax( "function" ,new caterwaul.syntax( "_f" ) ,new caterwaul.syntax( "(" ,new caterwaul.syntax( ";" ,new caterwaul.syntax( "_xs" ) ,new caterwaul.syntax( "_scope" ) ) ) ,new caterwaul.syntax( "{" ,new caterwaul.syntax( ";" ,new caterwaul.syntax( "[]" ,new caterwaul.syntax( "C" ) ,new caterwaul.syntax( "_scope" ) ) ,new caterwaul.syntax( "[]" ,new caterwaul.syntax( "S" ) ,new caterwaul.syntax( "_body" ) ) ) ) )'),qs1l:('new caterwaul.syntax( "[]" ,new caterwaul.syntax( "S" ) ,new caterwaul.syntax( "try" ,new caterwaul.syntax( "{" ,new caterwaul.syntax( "_x" ) ) ,new caterwaul.syntax( "catch" ,new caterwaul.syntax( "(" ,new caterwaul.syntax( "_e" ) ) ,new caterwaul.syntax( "_y" ) ,new caterwaul.syntax( "finally" ,new caterwaul.syntax( "_z" ) ) ) ) )'),qs1m:('new caterwaul.syntax( "try" ,new caterwaul.syntax( "{" ,new caterwaul.syntax( "[]" ,new caterwaul.syntax( "S" ) ,new caterwaul.syntax( "_x" ) ) ) ,new caterwaul.syntax( "catch" ,new caterwaul.syntax( "(" ,new caterwaul.syntax( "_e" ) ) ,new caterwaul.syntax( "[]" ,new caterwaul.syntax( "S" ) ,new caterwaul.syntax( "_y" ) ) ,new caterwaul.syntax( "finally" ,new caterwaul.syntax( "[]" ,new caterwaul.syntax( "S" ) ,new caterwaul.syntax( "_z" ) ) ) ) )'),qs1n:('new caterwaul.syntax( "[]" ,new caterwaul.syntax( "S" ) ,new caterwaul.syntax( "try" ,new caterwaul.syntax( "{" ,new caterwaul.syntax( "_x" ) ) ,new caterwaul.syntax( "catch" ,new caterwaul.syntax( "(" ,new caterwaul.syntax( "_e" ) ) ,new caterwaul.syntax( "_y" ) ) ) )'),qs1o:('new caterwaul.syntax( "try" ,new caterwaul.syntax( "{" ,new caterwaul.syntax( "[]" ,new caterwaul.syntax( "S" ) ,new caterwaul.syntax( "_x" ) ) ) ,new caterwaul.syntax( "catch" ,new caterwaul.syntax( "(" ,new caterwaul.syntax( "_e" ) ) ,new caterwaul.syntax( "[]" ,new caterwaul.syntax( "S" ) ,new caterwaul.syntax( "_y" ) ) ) )'),qs1p:('new caterwaul.syntax( "[]" ,new caterwaul.syntax( "S" ) ,new caterwaul.syntax( "i;" ,new caterwaul.syntax( "try" ,new caterwaul.syntax( "{" ,new caterwaul.syntax( "_x" ) ) ) ,new caterwaul.syntax( "finally" ,new caterwaul.syntax( "_y" ) ) ) )'),qs1q:('new caterwaul.syntax( "i;" ,new caterwaul.syntax( "try" ,new caterwaul.syntax( "{" ,new caterwaul.syntax( "[]" ,new caterwaul.syntax( "S" ) ,new caterwaul.syntax( "_x" ) ) ) ) ,new caterwaul.syntax( "finally" ,new caterwaul.syntax( "[]" ,new caterwaul.syntax( "S" ) ,new caterwaul.syntax( "_y" ) ) ) )'),qs1r:('new caterwaul.syntax( "[]" ,new caterwaul.syntax( "L" ) ,new caterwaul.syntax( "_x@0" ) )'),qs1s:('new caterwaul.syntax( "_x" )'),qs1t:('new caterwaul.syntax( "[]" ,new caterwaul.syntax( "L" ) ,new caterwaul.syntax( "[]" ,new caterwaul.syntax( "_x" ) ,new caterwaul.syntax( "_y" ) ) )'),qs1u:('new caterwaul.syntax( "[]" ,new caterwaul.syntax( "[]" ,new caterwaul.syntax( "R" ) ,new caterwaul.syntax( "_x" ) ) ,new caterwaul.syntax( "[]" ,new caterwaul.syntax( "L" ) ,new caterwaul.syntax( "_y" ) ) )'),qs1v:('new caterwaul.syntax( "[]" ,new caterwaul.syntax( "L" ) ,new caterwaul.syntax( "." ,new caterwaul.syntax( "_x" ) ,new caterwaul.syntax( "_y" ) ) )'),qs1w:('new caterwaul.syntax( "." ,new caterwaul.syntax( "[]" ,new caterwaul.syntax( "R" ) ,new caterwaul.syntax( "_x" ) ) ,new caterwaul.syntax( "_y" ) )'),qs1x:('new caterwaul.syntax( "[]" ,new caterwaul.syntax( "L" ) ,new caterwaul.syntax( "()" ,new caterwaul.syntax( "_x" ) ,new caterwaul.syntax( "_y" ) ) )'),qs1y:('new caterwaul.syntax( "[]" ,new caterwaul.syntax( "U" ) ,new caterwaul.syntax( "()" ,new caterwaul.syntax( "_x" ) ,new caterwaul.syntax( "_y" ) ) )'),ahead:("(function(xs) {var x,x0,xi,xl,xr;\nfor(var xr=new xs.constructor() ,xi=0,xl=xs.length;\nxi<xl;\n ++xi)x=xs[xi] ,xr.push( ( [caterwaul.parse( ( 'R[_x ' + (x) + ' _y]' ) ) ,caterwaul.parse( ( 'H[R[_x] ' + (x) + ' R[_y]]' ) ) ] ) ) ;\nreturn xr} ) .call(this, [ '+' , '-' , '*' , '/' , '%' , '<<' , '>>' , '>>>' , '<' , '>' , '<=' , '>=' , 'instanceof' , 'in' , '==' , '!=' , '===' , '!==' , '&' , '^' , '|' , '&&' , '||' ] )"),ahead1:("(function(xs) {var x,x0,xi,xl,xr;\nfor(var xr=new xs.constructor() ,xi=0,xl=xs.length;\nxi<xl;\n ++xi)x=xs[xi] ,xr.push( ( [caterwaul.parse( ( 'R[_x ' + (x) + ' _y]' ) ) ,caterwaul.parse( ( 'H[L[_x] ' + (x) + ' R[_y]]' ) ) ] ) ) ;\nreturn xr} ) .call(this, [ '=' , '+=' , '-=' , '*=' , '/=' , '%=' , '<<=' , '>>=' , '>>>=' , '&=' , '|=' , '^=' ] )"),qs1z:('new caterwaul.syntax( "[]" ,new caterwaul.syntax( "R" ) ,new caterwaul.syntax( "_x@0" ) )'),qs20:('new caterwaul.syntax( "[]" ,new caterwaul.syntax( "H" ) ,new caterwaul.syntax( "_x" ) )'),qs21:('new caterwaul.syntax( "[]" ,new caterwaul.syntax( "R" ) ,new caterwaul.syntax( "" ) )'),qs22:('new caterwaul.syntax( "" )'),qs23:('new caterwaul.syntax( "[]" ,new caterwaul.syntax( "R" ) ,new caterwaul.syntax( "delete" ,new caterwaul.syntax( "_x" ) ) )'),qs24:('new caterwaul.syntax( "[]" ,new caterwaul.syntax( "H" ) ,new caterwaul.syntax( "delete" ,new caterwaul.syntax( "[]" ,new caterwaul.syntax( "L" ) ,new caterwaul.syntax( "_x" ) ) ) )'),qs25:('new caterwaul.syntax( "[]" ,new caterwaul.syntax( "R" ) ,new caterwaul.syntax( "new" ,new caterwaul.syntax( "()" ,new caterwaul.syntax( "_f" ) ,new caterwaul.syntax( "_xs" ) ) ) )'),qs26:('new caterwaul.syntax( "new" ,new caterwaul.syntax( "()" ,new caterwaul.syntax( "_f" ) ,new caterwaul.syntax( "[]" ,new caterwaul.syntax( "R" ) ,new caterwaul.syntax( "_xs" ) ) ) )'),qs27:('new caterwaul.syntax( "[]" ,new caterwaul.syntax( "R" ) ,new caterwaul.syntax( "." ,new caterwaul.syntax( "_x" ) ,new caterwaul.syntax( "_y" ) ) )'),qs28:('new caterwaul.syntax( "[]" ,new caterwaul.syntax( "H" ) ,new caterwaul.syntax( "." ,new caterwaul.syntax( "[]" ,new caterwaul.syntax( "R" ) ,new caterwaul.syntax( "_x" ) ) ,new caterwaul.syntax( "_y" ) ) )'),qs29:('new caterwaul.syntax( "[]" ,new caterwaul.syntax( "R" ) ,new caterwaul.syntax( "(" ,new caterwaul.syntax( "_x" ) ) )'),qs2a:('new caterwaul.syntax( "(" ,new caterwaul.syntax( "[]" ,new caterwaul.syntax( "R" ) ,new caterwaul.syntax( "_x" ) ) )'),qs2b:('new caterwaul.syntax( "[]" ,new caterwaul.syntax( "R" ) ,new caterwaul.syntax( "[]" ,new caterwaul.syntax( "_x" ) ,new caterwaul.syntax( "_y" ) ) )'),qs2c:('new caterwaul.syntax( "[]" ,new caterwaul.syntax( "H" ) ,new caterwaul.syntax( "[]" ,new caterwaul.syntax( "[]" ,new caterwaul.syntax( "R" ) ,new caterwaul.syntax( "_x" ) ) ,new caterwaul.syntax( "[]" ,new caterwaul.syntax( "R" ) ,new caterwaul.syntax( "_y" ) ) ) )'),qs2d:('new caterwaul.syntax( "[]" ,new caterwaul.syntax( "R" ) ,new caterwaul.syntax( "[" ,new caterwaul.syntax( "_x" ) ) )'),qs2e:('new caterwaul.syntax( "[]" ,new caterwaul.syntax( "H" ) ,new caterwaul.syntax( "[" ,new caterwaul.syntax( "[]" ,new caterwaul.syntax( "R" ) ,new caterwaul.syntax( "_x" ) ) ) )'),qs2f:('new caterwaul.syntax( "[]" ,new caterwaul.syntax( "R" ) ,new caterwaul.syntax( "," ,new caterwaul.syntax( "_x" ) ,new caterwaul.syntax( "_y" ) ) )'),qs2g:('new caterwaul.syntax( "," ,new caterwaul.syntax( "[]" ,new caterwaul.syntax( "R" ) ,new caterwaul.syntax( "_x" ) ) ,new caterwaul.syntax( "[]" ,new caterwaul.syntax( "R" ) ,new caterwaul.syntax( "_y" ) ) )'),qs2h:('new caterwaul.syntax( "[]" ,new caterwaul.syntax( "R" ) ,new caterwaul.syntax( "{" ,new caterwaul.syntax( "_x" ) ) )'),qs2i:('new caterwaul.syntax( "[]" ,new caterwaul.syntax( "H" ) ,new caterwaul.syntax( "{" ,new caterwaul.syntax( "[]" ,new caterwaul.syntax( "R" ) ,new caterwaul.syntax( "_x" ) ) ) )'),qs2j:('new caterwaul.syntax( "[]" ,new caterwaul.syntax( "R" ) ,new caterwaul.syntax( "()" ,new caterwaul.syntax( "_x" ) ,new caterwaul.syntax( "_y" ) ) )'),qs2k:('new caterwaul.syntax( "[]" ,new caterwaul.syntax( "H" ) ,new caterwaul.syntax( "()" ,new caterwaul.syntax( "[]" ,new caterwaul.syntax( "R" ) ,new caterwaul.syntax( "_x" ) ) ,new caterwaul.syntax( "[]" ,new caterwaul.syntax( "R" ) ,new caterwaul.syntax( "_y" ) ) ) )'),qs2l:('new caterwaul.syntax( "[]" ,new caterwaul.syntax( "R" ) ,new caterwaul.syntax( "()" ,new caterwaul.syntax( "." ,new caterwaul.syntax( "_x" ) ,new caterwaul.syntax( "_y" ) ) ,new caterwaul.syntax( "_z" ) ) )'),qs2m:('new caterwaul.syntax( "[]" ,new caterwaul.syntax( "H" ) ,new caterwaul.syntax( "()" ,new caterwaul.syntax( "." ,new caterwaul.syntax( "[]" ,new caterwaul.syntax( "R" ) ,new caterwaul.syntax( "_x" ) ) ,new caterwaul.syntax( "_y" ) ) ,new caterwaul.syntax( "[]" ,new caterwaul.syntax( "R" ) ,new caterwaul.syntax( "_z" ) ) ) )'),qs2n:('new caterwaul.syntax( "[]" ,new caterwaul.syntax( "R" ) ,new caterwaul.syntax( "()" ,new caterwaul.syntax( "[]" ,new caterwaul.syntax( "_x" ) ,new caterwaul.syntax( "_y" ) ) ,new caterwaul.syntax( "_z" ) ) )'),qs2o:('new caterwaul.syntax( "[]" ,new caterwaul.syntax( "H" ) ,new caterwaul.syntax( "()" ,new caterwaul.syntax( "[]" ,new caterwaul.syntax( "[]" ,new caterwaul.syntax( "R" ) ,new caterwaul.syntax( "_x" ) ) ,new caterwaul.syntax( "[]" ,new caterwaul.syntax( "R" ) ,new caterwaul.syntax( "_y" ) ) ) ,new caterwaul.syntax( "[]" ,new caterwaul.syntax( "R" ) ,new caterwaul.syntax( "_z" ) ) ) )'),qs2p:('new caterwaul.syntax( "[]" ,new caterwaul.syntax( "R" ) ,new caterwaul.syntax( "void" ,new caterwaul.syntax( "_x" ) ) )'),qs2q:('new caterwaul.syntax( "void" ,new caterwaul.syntax( "[]" ,new caterwaul.syntax( "R" ) ,new caterwaul.syntax( "_x" ) ) )'),qs2r:('new caterwaul.syntax( "[]" ,new caterwaul.syntax( "R" ) ,new caterwaul.syntax( "typeof" ,new caterwaul.syntax( "_x" ) ) )'),qs2s:('new caterwaul.syntax( "[]" ,new caterwaul.syntax( "H" ) ,new caterwaul.syntax( "typeof" ,new caterwaul.syntax( "[]" ,new caterwaul.syntax( "R" ) ,new caterwaul.syntax( "_x" ) ) ) )'),qs2t:('new caterwaul.syntax( "[]" ,new caterwaul.syntax( "R" ) ,new caterwaul.syntax( ":" ,new caterwaul.syntax( "_x" ) ,new caterwaul.syntax( "_y" ) ) )'),qs2u:('new caterwaul.syntax( ":" ,new caterwaul.syntax( "_x" ) ,new caterwaul.syntax( "[]" ,new caterwaul.syntax( "R" ) ,new caterwaul.syntax( "_y" ) ) )'),qs2v:('new caterwaul.syntax( "[]" ,new caterwaul.syntax( "R" ) ,new caterwaul.syntax( "typeof" ,new caterwaul.syntax( "_x@0" ) ) )'),qs2w:('new caterwaul.syntax( "[]" ,new caterwaul.syntax( "H" ) ,new caterwaul.syntax( "typeof" ,new caterwaul.syntax( "_x" ) ) )'),qs2x:('new caterwaul.syntax( "[]" ,new caterwaul.syntax( "R" ) ,new caterwaul.syntax( "u+" ,new caterwaul.syntax( "_x" ) ) )'),qs2y:('new caterwaul.syntax( "[]" ,new caterwaul.syntax( "H" ) ,new caterwaul.syntax( "u+" ,new caterwaul.syntax( "[]" ,new caterwaul.syntax( "R" ) ,new caterwaul.syntax( "_x" ) ) ) )'),qs2z:('new caterwaul.syntax( "[]" ,new caterwaul.syntax( "R" ) ,new caterwaul.syntax( "u~" ,new caterwaul.syntax( "_x" ) ) )'),qs30:('new caterwaul.syntax( "[]" ,new caterwaul.syntax( "H" ) ,new caterwaul.syntax( "u~" ,new caterwaul.syntax( "[]" ,new caterwaul.syntax( "R" ) ,new caterwaul.syntax( "_x" ) ) ) )'),qs31:('new caterwaul.syntax( "[]" ,new caterwaul.syntax( "R" ) ,new caterwaul.syntax( "u-" ,new caterwaul.syntax( "_x" ) ) )'),qs32:('new caterwaul.syntax( "[]" ,new caterwaul.syntax( "H" ) ,new caterwaul.syntax( "u-" ,new caterwaul.syntax( "[]" ,new caterwaul.syntax( "R" ) ,new caterwaul.syntax( "_x" ) ) ) )'),qs33:('new caterwaul.syntax( "[]" ,new caterwaul.syntax( "R" ) ,new caterwaul.syntax( "u!" ,new caterwaul.syntax( "_x" ) ) )'),qs34:('new caterwaul.syntax( "[]" ,new caterwaul.syntax( "H" ) ,new caterwaul.syntax( "u!" ,new caterwaul.syntax( "[]" ,new caterwaul.syntax( "R" ) ,new caterwaul.syntax( "_x" ) ) ) )'),qs35:('new caterwaul.syntax( "[]" ,new caterwaul.syntax( "R" ) ,new caterwaul.syntax( "u++" ,new caterwaul.syntax( "_x" ) ) )'),qs36:('new caterwaul.syntax( "[]" ,new caterwaul.syntax( "H" ) ,new caterwaul.syntax( "u++" ,new caterwaul.syntax( "[]" ,new caterwaul.syntax( "L" ) ,new caterwaul.syntax( "_x" ) ) ) )'),qs37:('new caterwaul.syntax( "[]" ,new caterwaul.syntax( "R" ) ,new caterwaul.syntax( "++" ,new caterwaul.syntax( "_x" ) ) )'),qs38:('new caterwaul.syntax( "[]" ,new caterwaul.syntax( "H" ) ,new caterwaul.syntax( "++" ,new caterwaul.syntax( "[]" ,new caterwaul.syntax( "L" ) ,new caterwaul.syntax( "_x" ) ) ) )'),qs39:('new caterwaul.syntax( "[]" ,new caterwaul.syntax( "R" ) ,new caterwaul.syntax( "u--" ,new caterwaul.syntax( "_x" ) ) )'),qs3a:('new caterwaul.syntax( "[]" ,new caterwaul.syntax( "H" ) ,new caterwaul.syntax( "u--" ,new caterwaul.syntax( "[]" ,new caterwaul.syntax( "L" ) ,new caterwaul.syntax( "_x" ) ) ) )'),qs3b:('new caterwaul.syntax( "[]" ,new caterwaul.syntax( "R" ) ,new caterwaul.syntax( "--" ,new caterwaul.syntax( "_x" ) ) )'),qs3c:('new caterwaul.syntax( "[]" ,new caterwaul.syntax( "H" ) ,new caterwaul.syntax( "--" ,new caterwaul.syntax( "[]" ,new caterwaul.syntax( "L" ) ,new caterwaul.syntax( "_x" ) ) ) )'),qs3d:('new caterwaul.syntax( "[]" ,new caterwaul.syntax( "R" ) ,new caterwaul.syntax( "?" ,new caterwaul.syntax( "_x" ) ,new caterwaul.syntax( "_y" ) ,new caterwaul.syntax( "_z" ) ) )'),qs3e:('new caterwaul.syntax( "[]" ,new caterwaul.syntax( "H" ) ,new caterwaul.syntax( "?" ,new caterwaul.syntax( "[]" ,new caterwaul.syntax( "R" ) ,new caterwaul.syntax( "_x" ) ) ,new caterwaul.syntax( "[]" ,new caterwaul.syntax( "R" ) ,new caterwaul.syntax( "_y" ) ) ,new caterwaul.syntax( "[]" ,new caterwaul.syntax( "R" ) ,new caterwaul.syntax( "_z" ) ) ) )'),qs3f:('new caterwaul.syntax( "[]" ,new caterwaul.syntax( "R" ) ,new caterwaul.syntax( "function" ,new caterwaul.syntax( "(" ,new caterwaul.syntax( ";" ,new caterwaul.syntax( "_xs" ) ,new caterwaul.syntax( "_scope" ) ) ) ,new caterwaul.syntax( "{" ,new caterwaul.syntax( "_body" ) ) ) )'),qs3g:('new caterwaul.syntax( "[]" ,new caterwaul.syntax( "H" ) ,new caterwaul.syntax( "function" ,new caterwaul.syntax( "(" ,new caterwaul.syntax( "_xs" ) ) ,new caterwaul.syntax( "{" ,new caterwaul.syntax( ";" ,new caterwaul.syntax( "[]" ,new caterwaul.syntax( "C" ) ,new caterwaul.syntax( "_scope" ) ) ,new caterwaul.syntax( "[]" ,new caterwaul.syntax( "S" ) ,new caterwaul.syntax( "_body" ) ) ) ) ) )'),qs3h:('new caterwaul.syntax( "[]" ,new caterwaul.syntax( "R" ) ,new caterwaul.syntax( "function" ,new caterwaul.syntax( "_f" ) ,new caterwaul.syntax( "(" ,new caterwaul.syntax( ";" ,new caterwaul.syntax( "_xs" ) ,new caterwaul.syntax( "_scope" ) ) ) ,new caterwaul.syntax( "{" ,new caterwaul.syntax( "_body" ) ) ) )'),qs3i:('new caterwaul.syntax( "[]" ,new caterwaul.syntax( "H" ) ,new caterwaul.syntax( "function" ,new caterwaul.syntax( "_f" ) ,new caterwaul.syntax( "(" ,new caterwaul.syntax( "_xs" ) ) ,new caterwaul.syntax( "{" ,new caterwaul.syntax( ";" ,new caterwaul.syntax( "[]" ,new caterwaul.syntax( "C" ) ,new caterwaul.syntax( "_scope" ) ) ,new caterwaul.syntax( "[]" ,new caterwaul.syntax( "S" ) ,new caterwaul.syntax( "_body" ) ) ) ) ) )'),qs3j:('new caterwaul.syntax( "()" ,new caterwaul.syntax( "hook" ) ,new caterwaul.syntax( "," ,new caterwaul.syntax( "_tree" ) ,new caterwaul.syntax( "&&" ,new caterwaul.syntax( "()" ,new caterwaul.syntax( "hook" ) ,new caterwaul.syntax( "_tree" ) ) ,new caterwaul.syntax( "(" ,new caterwaul.syntax( "_value" ) ) ) ) )'),qs3k:('new caterwaul.syntax( "()" ,new caterwaul.syntax( "hook" ) ,new caterwaul.syntax( "," ,new caterwaul.syntax( "_tree" ) ,new caterwaul.syntax( "(" ,new caterwaul.syntax( "_value" ) ) ) )'),qs3l:('new caterwaul.syntax( "[]" ,new caterwaul.syntax( "H" ) ,new caterwaul.syntax( "_x" ) )'),qs3m:('new caterwaul.syntax( "_x" )'),qs3n:('new caterwaul.syntax( "if" ,new caterwaul.syntax( "(" ,new caterwaul.syntax( "===" ,new caterwaul.syntax( "this" ) ,new caterwaul.syntax( "_hook" ) ) ) ,new caterwaul.syntax( "if" ,new caterwaul.syntax( "(" ,new caterwaul.syntax( "." ,new caterwaul.syntax( "arguments" ) ,new caterwaul.syntax( "length" ) ) ) ,new caterwaul.syntax( "return" ,new caterwaul.syntax( "_set" ) ) ,new caterwaul.syntax( "else" ,new caterwaul.syntax( "return" ,new caterwaul.syntax( "_get" ) ) ) ) )'),qs3o:('new caterwaul.syntax( "&&" ,new caterwaul.syntax( "in" ,new caterwaul.syntax( "_name" ) ,new caterwaul.syntax( "[]" ,new caterwaul.syntax( "arguments" ) ,new caterwaul.syntax( "0" ) ) ) ,new caterwaul.syntax( "(" ,new caterwaul.syntax( "=" ,new caterwaul.syntax( "_v" ) ,new caterwaul.syntax( "." ,new caterwaul.syntax( "[]" ,new caterwaul.syntax( "arguments" ) ,new caterwaul.syntax( "0" ) ) ,new caterwaul.syntax( "_v" ) ) ) ) )'),qs3p:('new caterwaul.syntax( "[]" ,new caterwaul.syntax( "C" ) ,new caterwaul.syntax( "_scope" ) )'),qs3q:('new caterwaul.syntax( "null" )'),qs3r:('new caterwaul.syntax( "." ,new caterwaul.syntax( "hook" ) ,new caterwaul.syntax( "trace_eval" ) )'),qs3s:('new caterwaul.syntax( "[]" ,new caterwaul.syntax( "H" ) ,new caterwaul.syntax( "()" ,new caterwaul.syntax( "eval" ) ,new caterwaul.syntax( "()" ,new caterwaul.syntax( "trace" ) ,new caterwaul.syntax( "[]" ,new caterwaul.syntax( "R" ) ,new caterwaul.syntax( "_x" ) ) ) ) )'),qs3t:('new caterwaul.syntax( "[]" ,new caterwaul.syntax( "H" ) ,new caterwaul.syntax( "new" ,new caterwaul.syntax( "()" ,new caterwaul.syntax( "Function" ) ,new caterwaul.syntax( "()" ,new caterwaul.syntax( "trace" ) ,new caterwaul.syntax( "[]" ,new caterwaul.syntax( "R" ) ,new caterwaul.syntax( "_x" ) ) ) ) ) )'),qs3u:('new caterwaul.syntax( "[]" ,new caterwaul.syntax( "H" ) ,new caterwaul.syntax( "new" ,new caterwaul.syntax( "()" ,new caterwaul.syntax( "Function" ) ,new caterwaul.syntax( "," ,new caterwaul.syntax( "[]" ,new caterwaul.syntax( "R" ) ,new caterwaul.syntax( "_xs" ) ) ,new caterwaul.syntax( "()" ,new caterwaul.syntax( "trace" ) ,new caterwaul.syntax( "[]" ,new caterwaul.syntax( "R" ) ,new caterwaul.syntax( "_x" ) ) ) ) ) ) )')};
return(L)}).call(this,new caterwaul.syntax("[]",new caterwaul.syntax("H"),new caterwaul.syntax("_x")),new caterwaul.syntax("[]",new caterwaul.syntax("U"),new caterwaul.syntax("_x")),new caterwaul.syntax(""),new caterwaul.syntax("[]",new caterwaul.syntax("S"),new caterwaul.syntax("_x")),new caterwaul.syntax("[]",new caterwaul.syntax("R"),new caterwaul.syntax("_x")),new caterwaul.syntax("[]",new caterwaul.syntax("S"),new caterwaul.syntax("{",new caterwaul.syntax("_x"))),new caterwaul.syntax("{",new caterwaul.syntax("[]",new caterwaul.syntax("S"),new caterwaul.syntax("_x"))),new caterwaul.syntax("[]",new caterwaul.syntax("S"),new caterwaul.syntax("i;",new caterwaul.syntax("_x"),new caterwaul.syntax("_y"))),new caterwaul.syntax("i;",new caterwaul.syntax("[]",new caterwaul.syntax("S"),new caterwaul.syntax("_x")),new caterwaul.syntax("[]",new caterwaul.syntax("S"),new caterwaul.syntax("_y"))),new caterwaul.syntax("[]",new caterwaul.syntax("S"),new caterwaul.syntax(";",new caterwaul.syntax("_x"),new caterwaul.syntax("_y"))),new caterwaul.syntax(";",new caterwaul.syntax("[]",new caterwaul.syntax("S"),new caterwaul.syntax("_x")),new caterwaul.syntax("[]",new caterwaul.syntax("S"),new caterwaul.syntax("_y"))),new caterwaul.syntax("[]",new caterwaul.syntax("S"),new caterwaul.syntax("var",new caterwaul.syntax(",",new caterwaul.syntax("_vs"),new caterwaul.syntax("_x")))),new caterwaul.syntax(",",new caterwaul.syntax("[]",new caterwaul.syntax("S"),new caterwaul.syntax("var",new caterwaul.syntax("_vs"))),new caterwaul.syntax("[]",new caterwaul.syntax("R"),new caterwaul.syntax("_x"))),new caterwaul.syntax("[]",new caterwaul.syntax("S"),new caterwaul.syntax("if",new caterwaul.syntax("(",new caterwaul.syntax("_cond")),new caterwaul.syntax("_body"))),new caterwaul.syntax("if",new caterwaul.syntax("(",new caterwaul.syntax("[]",new caterwaul.syntax("R"),new caterwaul.syntax("_cond"))),new caterwaul.syntax("[]",new caterwaul.syntax("S"),new caterwaul.syntax("_body"))),new caterwaul.syntax("[]",new caterwaul.syntax("S"),new caterwaul.syntax("var",new caterwaul.syntax(",",new caterwaul.syntax("_vs"),new caterwaul.syntax("_name@0")))),new caterwaul.syntax(";",new caterwaul.syntax("[]",new caterwaul.syntax("S"),new caterwaul.syntax("var",new caterwaul.syntax("_vs"))),new caterwaul.syntax("[]",new caterwaul.syntax("S"),new caterwaul.syntax("var",new caterwaul.syntax("_name")))),new caterwaul.syntax("[]",new caterwaul.syntax("S"),new caterwaul.syntax("if",new caterwaul.syntax("(",new caterwaul.syntax("_cond")),new caterwaul.syntax("_body"),new caterwaul.syntax("else",new caterwaul.syntax("_else")))),new caterwaul.syntax("if",new caterwaul.syntax("(",new caterwaul.syntax("[]",new caterwaul.syntax("R"),new caterwaul.syntax("_cond"))),new caterwaul.syntax("[]",new caterwaul.syntax("S"),new caterwaul.syntax("_body")),new caterwaul.syntax("else",new caterwaul.syntax("[]",new caterwaul.syntax("S"),new caterwaul.syntax("_else")))),new caterwaul.syntax("[]",new caterwaul.syntax("S"),new caterwaul.syntax("var",new caterwaul.syntax(",",new caterwaul.syntax("_vs"),new caterwaul.syntax("=",new caterwaul.syntax("_name"),new caterwaul.syntax("_value"))))),new caterwaul.syntax(";",new caterwaul.syntax("[]",new caterwaul.syntax("S"),new caterwaul.syntax("var",new caterwaul.syntax("_vs"))),new caterwaul.syntax("[]",new caterwaul.syntax("S"),new caterwaul.syntax("var",new caterwaul.syntax("=",new caterwaul.syntax("_name"),new caterwaul.syntax("_value"))))),new caterwaul.syntax("[]",new caterwaul.syntax("S"),new caterwaul.syntax("var",new caterwaul.syntax("_name@0"))),new caterwaul.syntax("var",new caterwaul.syntax("_name")),new caterwaul.syntax("[]",new caterwaul.syntax("S"),new caterwaul.syntax("throw",new caterwaul.syntax("_x"))),new caterwaul.syntax("throw",new caterwaul.syntax("[]",new caterwaul.syntax("R"),new caterwaul.syntax("_x"))),new caterwaul.syntax("[]",new caterwaul.syntax("S"),new caterwaul.syntax("var",new caterwaul.syntax("=",new caterwaul.syntax("_name"),new caterwaul.syntax("_value")))),new caterwaul.syntax("var",new caterwaul.syntax("=",new caterwaul.syntax("_name"),new caterwaul.syntax("[]",new caterwaul.syntax("R"),new caterwaul.syntax("_value")))),new caterwaul.syntax("[]",new caterwaul.syntax("S"),new caterwaul.syntax("var",new caterwaul.syntax("in",new caterwaul.syntax("_name"),new caterwaul.syntax("_value")))),new caterwaul.syntax("var",new caterwaul.syntax("in",new caterwaul.syntax("_name"),new caterwaul.syntax("[]",new caterwaul.syntax("R"),new caterwaul.syntax("_value")))),new caterwaul.syntax("[]",new caterwaul.syntax("S"),new caterwaul.syntax("for",new caterwaul.syntax("(",new caterwaul.syntax("_x")),new caterwaul.syntax("_body"))),new caterwaul.syntax("for",new caterwaul.syntax("(",new caterwaul.syntax("[]",new caterwaul.syntax("S"),new caterwaul.syntax("_x"))),new caterwaul.syntax("[]",new caterwaul.syntax("S"),new caterwaul.syntax("_body"))),new caterwaul.syntax("[]",new caterwaul.syntax("S"),new caterwaul.syntax("do",new caterwaul.syntax("_body"),new caterwaul.syntax("while",new caterwaul.syntax("(",new caterwaul.syntax("_cond"))))),new caterwaul.syntax("do",new caterwaul.syntax("S"),new caterwaul.syntax("[",new caterwaul.syntax("_body")),new caterwaul.syntax("while",new caterwaul.syntax("(",new caterwaul.syntax("[]",new caterwaul.syntax("R"),new caterwaul.syntax("_cond"))))),new caterwaul.syntax("[]",new caterwaul.syntax("S"),new caterwaul.syntax("while",new caterwaul.syntax("(",new caterwaul.syntax("_cond")),new caterwaul.syntax("_body"))),new caterwaul.syntax("while",new caterwaul.syntax("(",new caterwaul.syntax("[]",new caterwaul.syntax("R"),new caterwaul.syntax("_cond"))),new caterwaul.syntax("[]",new caterwaul.syntax("S"),new caterwaul.syntax("_body"))),new caterwaul.syntax("[]",new caterwaul.syntax("S"),new caterwaul.syntax("with",new caterwaul.syntax("(",new caterwaul.syntax("_x")),new caterwaul.syntax("_body"))),new caterwaul.syntax("with",new caterwaul.syntax("(",new caterwaul.syntax("[]",new caterwaul.syntax("R"),new caterwaul.syntax("_x"))),new caterwaul.syntax("[]",new caterwaul.syntax("S"),new caterwaul.syntax("_body"))),new caterwaul.syntax("[]",new caterwaul.syntax("S"),new caterwaul.syntax("continue",new caterwaul.syntax("_label"))),new caterwaul.syntax("continue",new caterwaul.syntax("_label")),new caterwaul.syntax("[]",new caterwaul.syntax("S"),new caterwaul.syntax("continue")),new caterwaul.syntax("continue"),new caterwaul.syntax("[]",new caterwaul.syntax("S"),new caterwaul.syntax("break",new caterwaul.syntax("_label"))),new caterwaul.syntax("break",new caterwaul.syntax("_label")),new caterwaul.syntax("[]",new caterwaul.syntax("S"),new caterwaul.syntax("break")),new caterwaul.syntax("break"),new caterwaul.syntax("[]",new caterwaul.syntax("S"),new caterwaul.syntax("return",new caterwaul.syntax("_x"))),new caterwaul.syntax("return",new caterwaul.syntax("[]",new caterwaul.syntax("R"),new caterwaul.syntax("_x"))),new caterwaul.syntax("[]",new caterwaul.syntax("S"),new caterwaul.syntax("return")),new caterwaul.syntax("return"),new caterwaul.syntax("[]",new caterwaul.syntax("S"),new caterwaul.syntax("for",new caterwaul.syntax("(",new caterwaul.syntax("in",new caterwaul.syntax("_name"),new caterwaul.syntax("_v"))),new caterwaul.syntax("_body"))),new caterwaul.syntax("for",new caterwaul.syntax("(",new caterwaul.syntax("in",new caterwaul.syntax("_name"),new caterwaul.syntax("[]",new caterwaul.syntax("R"),new caterwaul.syntax("_v")))),new caterwaul.syntax("_body")),new caterwaul.syntax("[]",new caterwaul.syntax("S"),new caterwaul.syntax("i;",new caterwaul.syntax("()",new caterwaul.syntax("switch"),new caterwaul.syntax("_value")),new caterwaul.syntax("{",new caterwaul.syntax("_cases")))),new caterwaul.syntax("i;",new caterwaul.syntax("()",new caterwaul.syntax("switch"),new caterwaul.syntax("[]",new caterwaul.syntax("R"),new caterwaul.syntax("_value"))),new caterwaul.syntax("{",new caterwaul.syntax("[]",new caterwaul.syntax("S"),new caterwaul.syntax("_cases")))),new caterwaul.syntax("[]",new caterwaul.syntax("S"),new caterwaul.syntax(":",new caterwaul.syntax("_x"),new caterwaul.syntax("_y"))),new caterwaul.syntax(":",new caterwaul.syntax("_x"),new caterwaul.syntax("[]",new caterwaul.syntax("S"),new caterwaul.syntax("_y"))),new caterwaul.syntax("[]",new caterwaul.syntax("S"),new caterwaul.syntax("function",new caterwaul.syntax("_f"),new caterwaul.syntax("(",new caterwaul.syntax(";",new caterwaul.syntax("_xs"),new caterwaul.syntax("_scope"))),new caterwaul.syntax("{",new caterwaul.syntax("_body")))),new caterwaul.syntax("function",new caterwaul.syntax("_f"),new caterwaul.syntax("(",new caterwaul.syntax(";",new caterwaul.syntax("_xs"),new caterwaul.syntax("_scope"))),new caterwaul.syntax("{",new caterwaul.syntax(";",new caterwaul.syntax("[]",new caterwaul.syntax("C"),new caterwaul.syntax("_scope")),new caterwaul.syntax("[]",new caterwaul.syntax("S"),new caterwaul.syntax("_body"))))),new caterwaul.syntax("[]",new caterwaul.syntax("S"),new caterwaul.syntax("try",new caterwaul.syntax("{",new caterwaul.syntax("_x")),new caterwaul.syntax("catch",new caterwaul.syntax("(",new caterwaul.syntax("_e")),new caterwaul.syntax("_y"),new caterwaul.syntax("finally",new caterwaul.syntax("_z"))))),new caterwaul.syntax("try",new caterwaul.syntax("{",new caterwaul.syntax("[]",new caterwaul.syntax("S"),new caterwaul.syntax("_x"))),new caterwaul.syntax("catch",new caterwaul.syntax("(",new caterwaul.syntax("_e")),new caterwaul.syntax("[]",new caterwaul.syntax("S"),new caterwaul.syntax("_y")),new caterwaul.syntax("finally",new caterwaul.syntax("[]",new caterwaul.syntax("S"),new caterwaul.syntax("_z"))))),new caterwaul.syntax("[]",new caterwaul.syntax("S"),new caterwaul.syntax("try",new caterwaul.syntax("{",new caterwaul.syntax("_x")),new caterwaul.syntax("catch",new caterwaul.syntax("(",new caterwaul.syntax("_e")),new caterwaul.syntax("_y")))),new caterwaul.syntax("try",new caterwaul.syntax("{",new caterwaul.syntax("[]",new caterwaul.syntax("S"),new caterwaul.syntax("_x"))),new caterwaul.syntax("catch",new caterwaul.syntax("(",new caterwaul.syntax("_e")),new caterwaul.syntax("[]",new caterwaul.syntax("S"),new caterwaul.syntax("_y")))),new caterwaul.syntax("[]",new caterwaul.syntax("S"),new caterwaul.syntax("i;",new caterwaul.syntax("try",new caterwaul.syntax("{",new caterwaul.syntax("_x"))),new caterwaul.syntax("finally",new caterwaul.syntax("_y")))),new caterwaul.syntax("i;",new caterwaul.syntax("try",new caterwaul.syntax("{",new caterwaul.syntax("[]",new caterwaul.syntax("S"),new caterwaul.syntax("_x")))),new caterwaul.syntax("finally",new caterwaul.syntax("[]",new caterwaul.syntax("S"),new caterwaul.syntax("_y")))),new caterwaul.syntax("[]",new caterwaul.syntax("L"),new caterwaul.syntax("_x@0")),new caterwaul.syntax("_x"),new caterwaul.syntax("[]",new caterwaul.syntax("L"),new caterwaul.syntax("[]",new caterwaul.syntax("_x"),new caterwaul.syntax("_y"))),new caterwaul.syntax("[]",new caterwaul.syntax("[]",new caterwaul.syntax("R"),new caterwaul.syntax("_x")),new caterwaul.syntax("[]",new caterwaul.syntax("L"),new caterwaul.syntax("_y"))),new caterwaul.syntax("[]",new caterwaul.syntax("L"),new caterwaul.syntax(".",new caterwaul.syntax("_x"),new caterwaul.syntax("_y"))),new caterwaul.syntax(".",new caterwaul.syntax("[]",new caterwaul.syntax("R"),new caterwaul.syntax("_x")),new caterwaul.syntax("_y")),new caterwaul.syntax("[]",new caterwaul.syntax("L"),new caterwaul.syntax("()",new caterwaul.syntax("_x"),new caterwaul.syntax("_y"))),new caterwaul.syntax("[]",new caterwaul.syntax("U"),new caterwaul.syntax("()",new caterwaul.syntax("_x"),new caterwaul.syntax("_y"))),(function(c){var a,e,b,f,d;

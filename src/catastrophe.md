          ...
          tracer_for(options) = trace -where [default_options = {mocks: {},  allow_mock_annotations: true,  trace_native_eval_forms: true,       environment: {},  closures: true,
                                              ...
                                              trace           = compiler(settings /-$.merge/ {mocks: all_mocks, trace_log: [], error_log: [],
                                                                ...
                                                                /-$.merge/ wcapture [options                 = settings,
                                                                                     ...
                                                                                     find(t, t = $.parse(t)) = collection(trace_log %~![t /~match/ x.tree() && xi + 1] *[x - 1] -seq,
                                                                                                                          trace_log, t),
...
function's body. This string is compiled against several closure variables:
    ...
    r()   returns the trace record for a given syntax tree
    t()   returns the timestamp of the trace record for a given syntax tree
    dt()  when pre-tracing is enabled, returns the total amount of time that elapsed while evaluating the given syntax tree
...
This query shows you each input to f() followed by the corresponding output. You can use string interpolation like this because the mapping function is caterwaul-transformed under :all.
                                                  ...
                                                  iteration_context(x)    = this.pattern /~match/ this.xs[x].tree() /or [{}] / {x: this.xs[x]} /-$.merge/
                                                                            wcapture [xs = this.xs,  r(t)  = xs |~!r[ri <= x ? r.tree() === t : (ri = x + 1, 0)][r] |seq,
                                                                                      xi = x,        dt(t) = r0.time - r0.pre_time -where [r0 = r(t)],

                                                                                                     v(t)  = r(t).value,  _ = xs[xi].tree(),
                                                                                                     t(t)  = r(t).time],
                                                  ...
                                                  last(n)                 = this.select(this.slice(this.length - 1 - (n || 1))),
...
through to rvalue expressions; this reflects the fact that you can have an expression whose return value is not used.
                  ...
                  statements = ['S[_x]'.qs     /-r/ 'R[_x]'.qs,
                                ...
                                'S[var _vs, _name@0]'.qs         /-r/ 'S[var _vs]; S[var _name]'.qs,  'S[if (_cond) _body else _else]'.qs /-r/ 'if (R[_cond]) S[_body] else S[_else]'.qs,
                                'S[var _vs, _name = _value]'.qs  /-r/ 'S[var _vs]; S[var _name = _value]'.qs,
...
this expression arbitrarily.
                  ...
                  lvalues = ['L[_x@0]'.qs   /-r/ '_x'.qs,        'L[_x[_y]]'.qs /-r/ 'R[_x][L[_y]]'.qs,
...
or array entry separator.
                  ...
                  rvalues = ['R[_x@0]'.qs      /-r/ 'H[_x]'.qs,               'R[]'.qs            /-r/ ''.qs,                                   // empty case required for [] and {}
                             'R[delete _x]'.qs /-r/ 'H[delete L[_x]]'.qs,     'R[new _f(_xs)]'.qs /-r/ 'new _f(R[_xs])'.qs,                     // TODO: trace this with better granularity
...
get the evaluated trace record using post_trace.
            ...
            hook_for(options) = observe -where [observe(t, v)      = arguments.length === 2 ? resolve(t, v) : enqueue(t),
                                                enqueue(t)         = trace_log.push(bt = new record(t) /-$.merge/ {pre_time: +new Date(), pre_id: ++id, bt: bt}),
                                                resolve(t, v)      = trace_record(t, v) -then- trim_trace_log() /when [options.trace_log_size && trace_log.length > options.trace_log_size << 1]
                                                                                        -then- v,

                                                trace_record(t, v) = r /-$.merge/ {value: v, time: +new Date(), id: ++id} -then- trace_log /~push/ r /unless [options.pre_trace]
                                                ...
                                                dequeue(t)         = bt && bt.tree_id === t ? bt = bt.bt : dequeue_remote(t),
                                                dequeue_remote(t)  = error_log /~push/ bt -then [bt = find_sync(bt)] -where [find_sync(bt) = bt.tree_id === t ? bt : find_sync(bt.bt)],

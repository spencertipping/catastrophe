...
behavior within the context of the original program, but you can disable this feature by setting 'closures' to false. Here's an example of how to access closure state:

    var tracer = catastrophe();
    tracer(function () {
      var x = 10;
      var closure = function () {return x};
      console.log('initial value: ' + closure());
    })();

In the console:

    > closure = tracer.find('closure = _x').map('v(_x)')[0]
    > tracer.closure_state(closure)
    { x: 10 }
    > closure()
    10
    > tracer.closure_state(closure, {x: 'hello world'})
    > closure()
    'hello world'
    >

Note that closure annotation won't cross a trace boundary. That is, only variables inside the function given to the tracer will be considered as closure data; the rest are assumed to be
globals. If you're tracing at the toplevel this won't be a problem.
...
### Caterwaul integration

You can use this debugger to trace caterwaul extensions, including having it trace itself. All you need to do is redefine whichever module you want to annotate. So, for example, here's how
you can debug catastrophe itself:

    tracer.trace_module(catastrophe.caterwaul, 'catastrophe');

This is abstracted into its own function because caterwaul modules may contain expression ref closure state and preserving that state involves some extra work.

          replace_module(trace, c, module) = module /-c.module/ transformed -where [bound_tree  = new c.opaque_tree(module /!c.module) /!c.late_bound_tree,
                                                                                    transformed = c.compile(bound_tree /!trace, {caterwaul: c}, {gensym_renaming: false})],
          ...
          tracer_for(options) = trace -where [default_options = {mocks: {},  allow_mock_annotations: true,  trace_native_eval_forms: true,       environment: {},  closures: true,
                                              ...
                                              trace           = compiler(settings /-$.merge/ {mocks: all_mocks, trace_log: [], index: {},
                                                                ...
                                                                /-$.merge/ wcapture [options                 = settings,
                                                                                     ...
                                                                                     trace_module(c, module) = replace_module(this, c, module),
                                                                                     is_traced(f)            = f.toString() /~indexOf/ $.gensym_entropy() > -1,

                                                                                     closure_state(f, state) = state ? f.call(options.hook, state) : f.call(options.hook),
                                                                                     trace_log               = options.trace_log,
                                                                                     hook                    = options.hook -ocq- hook_for(options),
                                                                                     trace_eval              = options.global && hook.trace_eval -ocq- eval_tracer_for(self),
...
### Tree reduction
  ...
  returning the fully parsed tree.
          ...
          trace_grammar(options) = trace -where [grammar      = $.grammar('L U R C H S'.qw, {initial: 'S[_x]', fix: false}, cc),
                                                 ...
                                                 trace(tree)  = {_x: tree /!reduce /!index /-mark_all_closures/ options} /!grammar]

          -where [cc(rule, anon) = tracing_rules() + custom_rules() -seq

          -where [tracing_rules() = unreachables + statements + lvalues + rvalues + hooks + closures -seq,
                  custom_rules()  = options.mocks /pairs *[x[0] /!$.parse /!anon /-r/ process_mock(x[1] /!process_mock)] -seq
...
It's important to preserve tree node IDs across the annotation transform. This lets us reverse-map trace log entries to their corresponding tree nodes later on. In order to do this, we
implement a custom rule() function that observes the incoming ID and replicates it.

                  fix(f)  = result -where [result(x) = f.call(this, x) -re- this(it) /when.it],
                  r(x, y) = fix('H[_x]'.qs /~match/ y ? "f.call(this, _) -se [_[1].id(), it[1].id = _[1].id, when.it]".qf -where [f = rule(x, y)] : rule(x, y)),
...
through to rvalue expressions; this reflects the fact that you can have an expression whose return value is not used.
                  ...
                  statements = ['S[_x]'.qs     /-r/ 'R[_x]'.qs,                                                 // toplevel rvalue expression
                                ...
                                'S[var _vs, _name@0]'.qs         /-r/ 'S[var _vs], _name'.qs,         'S[if (_cond) _body else _else]'.qs /-r/ 'if (R[_cond]) S[_body] else S[_else]'.qs,
                                'S[var _vs, _name = _value]'.qs  /-r/ 'S[var _vs], _name = R[_value]'.qs,
                                'S[var _name@0]'.qs              /-r/ 'var _name'.qs,                 'S[throw _x]'.qs                    /-r/ 'throw R[_x]'.qs,
                                'S[var _name = _value]'.qs       /-r/ 'var _name = R[_value]'.qs,
                                'S[var _name in _value]'.qs      /-r/ 'var _name in R[_value]'.qs,    'S[for (_x) _body]'.qs              /-r/ 'for (S[_x]) S[_body]'.qs,

                                'S[do _body while (_cond)]'.qs   /-r/ 'do S[_body] while (R[_cond])'.qs,
                                'S[while (_cond) _body]'.qs      /-r/ 'while (R[_cond]) S[_body]'.qs,
                                'S[with (_x) _body]'.qs          /-r/ 'with (R[_x]) S[_body]'.qs,

                                'S[continue _label]'.qs          /-r/ 'continue _label'.qs,           'S[continue]'.qs                    /-r/ 'continue'.qs,
                                'S[break _label]'.qs             /-r/ 'break _label'.qs,              'S[break]'.qs                       /-r/ 'break'.qs,
                                'S[return _x]'.qs                /-r/ 'return R[_x]'.qs,              'S[return]'.qs                      /-r/ 'return'.qs,

                                'S[for (_name in _v) _body]'.qs  /-r/ 'for (_name in R[_v]) _body'.qs,          // pathological case; we don't want the 'in' turning to an R[]
                                ...
                                'S[function _f(_xs; _scope) {_body}'.qs   /-r/ 'function _f(_xs) {C[_scope]; S[_body]}'.qs,
                                ...
                                'S[try {_x} finally _y]'.qs               /-r/ 'try {S[_x]} finally S[_y]'.qs],
...
this expression arbitrarily.
                  ...
                  lvalues = ['L[_x@0]'.qs   /-r/ '_x'.qs,        'L[_x[_y]]'.qs /-r/ 'R[_x][R[_y]]'.qs,
                             'L[_x._y]'.qs  /-r/ 'R[_x]._y'.qs,  'L[_x(_y)]'.qs /-r/ 'U[_x(_y)]'.qs],           // case for legacy IE code; this should be unreachable
...
or array entry separator.
                  ...
                  rvalues = ['R[_x]'.qs        /-r/ 'H[_x]'.qs,               'R[]'.qs            /-r/ ''.qs,                                   // empty case required for [] and {}
                             'R[R[_x]]'.qs     /-r/ 'R[_x]'.qs,               'R[H[_x]]'.qs       /-r/ 'H[_x]'.qs,

                             'R[new _x]'.qs    /-r/ 'H[new (R[_x])]'.qs,      'R[new _f(_xs)]'.qs /-r/ 'H[new (R[_f])(R[_xs])]'.qs,

                             'R[delete _x]'.qs /-r/ 'H[delete L[_x]]'.qs,
                             'R[_x._y]'.qs     /-r/ 'H[R[_x]._y]'.qs,         'R[(_x)]'.qs        /-r/ '(R[_x])'.qs,
                             'R[_x[_y]]'.qs    /-r/ 'H[R[_x][R[_y]]]'.qs,     'R[[_x]]'.qs        /-r/ 'H[[R[_x]]]'.qs,
                             'R[_x, _y]'.qs    /-r/ 'R[_x], R[_y]'.qs,        'R[{_x}]'.qs        /-r/ 'H[{R[_x]}]'.qs,

                             'R[_x(_y)]'.qs    /-r/ 'H[R[_x](R[_y])]'.qs,
                             'R[_x._y(_z)]'.qs /-r/ 'H[R[_x]._y(R[_z])]'.qs,  'R[_x[_y](_z)]'.qs  /-r/ 'H[R[_x][R[_y]](R[_z])]'.qs,

                             'R[void _x]'.qs   /-r/ 'void R[_x]'.qs,          'R[typeof _x]'.qs   /-r/ 'H[typeof R[_x]]'.qs,
                             'R[_x: _y]'.qs    /-r/ '_x: R[_y]'.qs,           'R[typeof _x@0]'.qs /-r/ 'H[typeof _x]'.qs,

                             'R[+_x]'.qs       /-r/ 'H[+R[_x]]'.qs,           'R[~_x]'.qs         /-r/ 'H[~R[_x]]'.qs,
                             'R[-_x]'.qs       /-r/ 'H[-R[_x]]'.qs,           'R[!_x]'.qs         /-r/ 'H[!R[_x]]'.qs,
                             'R[++_x]'.qs      /-r/ 'H[++L[_x]]'.qs,          'R[_x++]'.qs        /-r/ 'H[L[_x]++]'.qs,
                             'R[--_x]'.qs      /-r/ 'H[--L[_x]]'.qs,          'R[_x--]'.qs        /-r/ 'H[L[_x]--]'.qs] +
...
The function's closure scope is simply its set of identifiers minus the ones that are local to it. Each of these sets is stored as an object on the 'function' node.
                ...
                closure_setter_case(v)   = '_name in arguments[0] && (_v = arguments[0]._v)'.qs /~replace/ {_name: $.syntax.from_string(v /!fix_key), _v: v /!fix_key},

                closure_state_object(vs) = vs /keys *[[x /!fix_key, new $.syntax(x /!fix_key)]] /object /seq /!$.syntax.from_object,
...
set differences to figure out which variables are actually closure variables as opposed to locals. Note that we prefix all closure variables with @ so that Caterwaul can continue to read
their 'constructor' property.
            ...
            local_scope(t)           = scope_closed_reach(visit, t /!$.parse) -then- scope
                                                                              -where [visit(n)       = visit_vars(n[0].flatten(',')) -when [n.data === 'var'],
                                                                                      visit_vars(vs) = vs[0].flatten('=')[0] -re [it.data === 'in' ? scope['@#{it[0].data}'] = it : normal(vs)],
                                                                                      normal(vs)     = vs *![scope['@' + x.flatten('=')[0].data] = x] -seq,
                                                                                      scope          = +t[0][0].flatten(',') %[x.data.length] *[['@' + x.data, x]] /object -seq],
...
Note that this code will break if the function's list of formals is nullary. It needs to be an actual 'empty' node rather than being no node at all.
          ...
          mark_closure(options)(f_node) = f_node -se [it[0][0] = new $.syntax(';', it[0][0] || $.empty, new $.metadata_node(f_node /!closure_scope -when- options.closures, 'scope'))],
          mark_all_closures(t, options) = t.collect("_.data === 'function'".qf) *!mark -seq -then- t -where [mark = mark_closure(options)],
...
can mock out other evaluation primitives as well. This is fairly simple to do; you just use the tracer on the code. For example:
    ...
    {mocks: {'some_eval_form(_code)': 'some_eval_form(my_tracer(_code))'.qs /~replace/ {my_tracer: new $.ref(tracer)}}}
...
instead of functions.
            ...
            eval_mocks_for(hook_name) = {'R[eval(_x)]':              'H[eval(trace(R[_x]))]'.qs,
                                         ...
                                         'R[new Function(_xs, _x)]': 'H[new Function(R[_xs], trace(R[_x]))]'.qs} %v*[x /~replace/ {trace: trace_call}] -seq
                                        -where [trace_call = 'hook.trace_eval'.qs /~replace/ {hook: hook_name}],

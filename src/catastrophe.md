          ...
          tracer_for(options) = trace -where [default_options = {mocks: {},           allow_mock_annotations: true,  trace_native_eval_forms: true,       environment: {},
                                              ...
                                              trace           = compiler(settings /-$.merge/ {mocks: all_mocks, trace_log: [], error_log: [],
                                                                ...
                                                                /-$.merge/ wcapture [options                 = settings,
                                                                                     ...
                                                                                     find(t, t = $.parse(t)) = collection(trace_log %~![t /~match/ x.tree && xi] -seq, trace_log, t),
...
function's body. This string is compiled against several closure variables:
    ...
    x   the current event record; contains things like .tree, .value, .time, etc
...
So, for example, here's a basic query:

    tracer.find('f(_x)').map('_x + ": " + v(_)')

This query shows you each input to f() followed by the corresponding output.
                                                   ...
                                                   step(n)                        = this.select(this *[x + n] -seq),
                                                   adjacent(n)                    = this.select(this *~![ni[x - n, x + n] -seq] -seq),

...
It is important that the hook function return the value it is tracing. Otherwise the program's semantics will be changed!
                ...
                hook  = options.hook ? new $.ref(options.hook, 'catastrophe_hook') : new $.syntax(options.hook_name),
                hooks = []]],

        tracer_for(options) = trace -where [default_options = {mocks: {}, trace_mocks: true, allow_mock_annotations: true, environment: {}, pre_trace: true, trace: true,
                                                               hook_name: 'catastrophe_hook' /!$.gensym, hook: null},

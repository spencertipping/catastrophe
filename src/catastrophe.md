          ...
          tracer_for(options) = trace -where [default_options = {mocks: {},           allow_mock_annotations: true,  trace_native_eval_forms: true,       environment: {},
                                              ...
                                              trace           = compiler(settings /-$.merge/ {mocks: all_mocks, trace_log: [], error_log: [],
                                                                ...
                                                                /-$.merge/ wcapture [options                 = settings,
                                                                                     ...
                                                                                     find(t, t = $.parse(t)) = collection(trace_log %~![t /~match/ remove_markers_from(x.tree) && xi] -seq,
                                                                                                                          trace_log, t),
...
### Making trees presentable
  ...
  By default, all of the trees referenced by syntax trees will have tons of intermediate markers in them. This function gets rid of that.
...
bound in the selectors you use later on.
          ...
          collection_methods           = wcapture [function_compiler            = $(':all'),
                                                   ...
                                                   compile_string(s)            = s / this.match_variables_for(this.pattern) /-function_compiler/ {unbound_closure: true},

                                                   match_variables_for(pattern) = ('_ xs xi r v t dt'.qw + pattern.collect("_.is_wildcard()".qf)) *[[x, true]] /object -seq,
...
function's body. This string is compiled against several closure variables:
    ...
    xi    the index of the current event record in the log
...
This query shows you each input to f() followed by the corresponding output. You can use string interpolation like this because the mapping function is caterwaul-transformed under :all.
                                                   ...
                                                   to  (t, t = $.parse(t))        = this.select(this *~!o[ni[o, this.xs |  [xi > o ? t /~match/ x.tree : (xi = o, 0)][xi] |seq]    -seq] -seq),
                                                   from(t, t = $.parse(t))        = this.select(this *~!o[ni[   this.xs |~![xi < o ? t /~match/ x.tree : (xi = o, 0)][xi] |seq, o] -seq] -seq)],
...
through to rvalue expressions; this reflects the fact that you can have an expression whose return value is not used.
                  ...
                  statements = ['S[_x]'.qs     /-rule/ 'R[_x]'.qs,
                                ...
                                'S[var _vs, _x]'.qs              /-rule/ 'S[var _vs], R[_x]'.qs,         'S[if (_cond) _body]'.qs            /-rule/ 'if (R[_cond]) S[_body]'.qs,
                                'S[var _vs, _name@0]'.qs         /-rule/ 'S[var _vs]; S[var _name]'.qs,  'S[if (_cond) _body else _else]'.qs /-rule/ 'if (R[_cond]) S[_body] else S[_else]'.qs,
                                'S[var _vs, _name = _value]'.qs  /-rule/ 'S[var _vs]; S[var _name = _value]'.qs,
                                'S[var _name@0]'.qs              /-rule/ 'var _name'.qs,                 'S[throw _x]'.qs                    /-rule/ 'throw R[_x]'.qs,
                                'S[var _name = _value]'.qs       /-rule/ 'var _name = R[_value]'.qs,
                                'S[var _name in _value]'.qs      /-rule/ 'var _name in R[_value]'.qs,    'S[for (_x) _body]'.qs              /-rule/ 'for (S[_x]) S[_body]'.qs,
                                ...
                                'S[continue _label]'.qs          /-rule/ 'continue _label'.qs,           'S[continue]'.qs                    /-rule/ 'continue'.qs,
                                'S[break _label]'.qs             /-rule/ 'break _label'.qs,              'S[break]'.qs                       /-rule/ 'break'.qs,
                                'S[return _x]'.qs                /-rule/ 'return R[_x]'.qs,              'S[return]'.qs                      /-rule/ 'return'.qs,

                                'S[for (_name in _v) _body]'.qs  /-rule/ 'for (_name in R[_v]) _body'.qs,       // pathological case; we don't want the 'in' turning to an R[]
...
or array entry separator.
                  ...
                  rvalues = ['R[_x@0]'.qs      /-rule/ 'H[_x]'.qs,               'R[]'.qs            /-rule/ ''.qs,                             // empty case required for [] and {}
                            ...
                            ['R[_x ? _y : _z]'.qs /-rule/ 'H[R[_x] ? R[_y] : R[_z]]'.qs, 'R[function (_xs) {_body}]'.qs    /-rule/ 'function (_xs) {C; S[_body]}'.qs,
                                                                                         'R[function _f (_xs) {_body}]'.qs /-rule/ 'function _f (_xs) {C; S[_body]}'.qs] -seq
...
be unused.
                ...
                hook(t)                = t.is_constant() || (options.patterns && options.patterns |![x /~match/ remove_markers_from(t)] |seq) ? t :
                                           (options.pre_trace ? pre_hook_form : hook_form) /~replace/ {_x: new $.ref(t), _value: t},
